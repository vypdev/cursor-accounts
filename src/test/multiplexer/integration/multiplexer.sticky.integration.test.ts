...
let nextPort = 19_100 + (process.pid % 1000);
...
describe('Multiplexer sticky routing integration', () => {
  ...
  it('creates a sticky session binding after CONNECT', async () => {
    const routerPort = allocatePort();
    const upstreamA = allocatePort();
    const upstreamB = allocatePort();
    upstreamServers.push(await startMockUpstream(upstreamA));
    upstreamServers.push(await startMockUpstream(upstreamB));

    const config: MultiplexerConfig = {
      router: { port: routerPort, host: '127.0.0.1' },
      upstreams: [
        { id: 'u1', host: '127.0.0.1', port: upstreamA },
        { id: 'u2', host: '127.0.0.1', port: upstreamB },
      ],
    ...
    const connected = await connectViaProxy(routerPort);
    const sessions = runtime.sessionStore.list();

    await runtime.service.stop();

    assert.equal(connected, true);
    assert.equal(sessions.length, 1);
    assert.ok(['u1', 'u2'].includes(sessions[0]!.upstreamId));
  });

  it('assigns different upstreams for different source ports', async () => {
    const routerPort = allocatePort();
    const upstreamA = allocatePort();
    const upstreamB = allocatePort();
    upstreamServers.push(await startMockUpstream(upstreamA));
    ...
    await connectViaProxy(routerPort);
    await connectViaProxy(routerPort);

    const upstreamIds = new Set(
      runtime.sessionStore.list().map((binding) => binding.upstreamId)
    );
