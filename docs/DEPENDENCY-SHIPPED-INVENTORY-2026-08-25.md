...
The earlier current-target clean build produced a 28 MiB `darwin-arm64` VSIX
with 3,085 entries. It contained the Electron ABI 128
...
The target-specific binding checkpoint on 2026-08-26 produced the current
49,430,263-byte (47.14 MiB) `darwin-arm64` VSIX with 12,360 ZIP entries. It
downloaded and verified the official
`better-sqlite3-v12.9.0-electron-v128-darwin-arm64.tar.gz` asset, confirmed the
native binary target inside the VSIX, and passed the complete VSIX verifier.
...
11.1.0 feature. Current signature, SBOM, and license evidence is maintained in
[SUPPLY-CHAIN-EVIDENCE-2026-08-25.md](SUPPLY-CHAIN-EVIDENCE-2026-08-25.md),
including the automated `audit:supply-chain` command.
...
The clean-room artifact from this checkpoint was 49,429,715 bytes with 12,360
ZIP entries and 700 package manifests. It contains the runtime SDK, `undici`,
`bindings`, the Electron ABI 128 arm64 `better_sqlite3.node`, and the
efficiency migrations; no package-manager store is shipped. The latest
target-specific artifact is recorded above with its exact byte count and
embedded target verification. This closes the clean-room/current-target
portion of the evidence but does not close QA-2 or QA-3: license/SBOM policy
review and the complete supported-target matrix remain open.
