...
| `cursorAccounts.proxy.showLiveUsageInStatusBar` | `true` | Show live agent token counter + rough cost estimate (via proxy WebSocket API, not log tailing) |
| `cursorAccounts.proxy.estimatedDollarsPerMillionTokens` | `4` | Flat rate for live cost estimate (not real billing) |
| `cursorAccounts.proxy.developmentMode` | `false` | Write JSONL logs under `~/.cursor-accounts/proxy/logs/` (optional; traffic always flows via API) |
...
**Runtime modes:** By default, the extension host receives decoded traffic from the proxy localhost WebSocket API (`ProxyManager.onTraffic`). JSONL files and log tailing are only used when `developmentMode` is `true` or when you open the output channel with **Proxy: Output Tail From Start** (replays the active log file if one exists).
