...
| Latest implementation commit | 387bfa7 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 412 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `387bfa7`
...
decision isolation, and certificate material generation extraction with
single-flight initialization, followed by strict propagation of non-missing
certificate material read failures and status-bar display-policy extraction.
The
complete audit
included:

- extension-host tests: passed across the compiled host test suite;
- 1,193 host tests and coverage of 89.68% lines, 82.69% branches, and 85.87%
  functions;
- architecture rules and fixtures: passed for 413 production TypeScript files;
...
`387bfa7` using `--code-only` and no clustering. It scanned 860 code files and
contains 5,159 nodes and 14,560 raw edges; the directed post-build diagnostic
graph contains 12,409 edges.
...
directed multigraph diagnostic reports 2,006 dangling endpoint edges, one
self-loop, no missing endpoints, and 144 same-endpoint relation groups; it
retains 12,554 valid candidate edges and collapses them to 12,409 directed
...
The latest Repowise index covers the code tree after `387bfa7`; the host and
...
366 file entries (333 exact and 33 resolved mappings); the retained coverage
index reports 366 files, 90.09% lines, and 82.15% branches.
Detailed health reports average health at 8.92/10, hotspot health at 6.79/10,
...
the next proxy decoder hotspot, and the open physical/release recovery
evidence.
