...
|---|---|---|---|---|
| QA-0 | Baseline and finding reclassification | COMPLETE | `pnpm run audit` passes at `87237c6`; current Graphify/Repowise/dependency evidence is captured on 2026-08-26 at `87237c6` | Keep this baseline immutable and update it after every cross-cutting change |
| QA-1 | CI, tests, localization, lint | PARTIAL | Full audit passes with 972 host tests, 83.70% lines, 77.17% branches, and 81.20% functions; localization is 26 locales/428 keys and webview is 51/51 across 13 files with 84.97% lines, 78.24% branches, 83.87% functions, and explicit risk-based floors; the host suite also passes independently under the Node 24 contract; the Cursor protobuf extractor has an additional 6/6 hermetic contract suite, the Agent Tracking ingestion policy has 4/4 direct tests, token resolution/OAuth transport has 19/19 focused tests, and multi-profile quota boundaries have 20/20 focused tests | Add repeated host/webview-run evidence and review generated-artifact/source-lint policy |
...
| Latest implementation commit | 87237c6 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 340 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest `pnpm run audit` passed on 2026-08-26 at commit `87237c6` with:

- extension-host tests: passed across the compiled host test suite;
- 972 host tests and coverage of 83.70% lines, 77.17% branches, and 81.20%
  functions;
- architecture rules and fixtures: passed for 340 production TypeScript files;
...
`87237c6` using no clustering. It contains 5,786 nodes and 13,970 raw edges,
and the post-build diagnostic graph contains 12,020 edges.
...
diagnostic reports 1,740 dangling endpoint edges, one self-loop, no missing
...
The latest Repowise index covers the code tree at `87237c6`; the host and
webview lcov reports were explicitly merged with `repowise coverage add
coverage/lcov.info webview/coverage/lcov.info`. The coverage index reports 297
files after resolving the merged inputs (273 exact and 24 resolved mappings).
Detailed health reports average health
at 8.69/10, hotspot health at 6.25/10, worst-performer health at 4.37/10 for
`src/ui/accountsPanel.ts`, maintainability at 9.5/10, and performance at
9.9/10.
The current report contains 1,468 marker findings and 107 performance findings.
