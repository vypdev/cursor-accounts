...
| Latest implementation commit | 453e31a |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `453e31a`
...
- 1,112 host tests and coverage of 86.85% lines, 81.02% branches, and 84.56%
  functions;
- architecture rules and fixtures: passed for 394 production TypeScript files;
...
`453e31a` using `--code-only` and no clustering. It scanned 832 code files and
contains 5,009 nodes and 14,089 raw edges; the directed post-build diagnostic
graph contains 11,999 edges.
...
directed multigraph diagnostic reports 1,948 dangling endpoint edges, one
self-loop, no missing endpoints, and 141 same-endpoint relation groups; it
retains 12,141 valid candidate edges and collapses them to 11,999 directed
...
The latest Repowise index covers the code tree after `453e31a`; the host and
...
348 file entries (315 exact and 33 resolved mappings); the retained coverage
index reports 347 files, 88.56% lines, and 81.17% branches.
Detailed health reports average health at 8.87/10, hotspot health at 6.72/10,
worst-performer health at 4.15/10 for `packages/types/src/index.ts`,
maintainability at 9.51/10 overall with a 9.09/10 hotspot score, and
...
Repowise sync metadata points to commit `42ed7a8`, with no embedding provider or
