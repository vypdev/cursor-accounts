...
| QA-8 | Local test confidence | PARTIAL | Full audit passes 1,193 host tests and 69 webview tests across 18 files; overall host c8 is 89.78% lines/82.79% branches/85.97% functions and webview V8 coverage is 93.96% lines/84.85% branches/92.54% functions; the webview gate enforces global 40/70/80/40 floors plus tested-boundary floors for the reducer, VS Code bridge, boot-error renderer, ProfileCard, each extracted ProfileCard section, the message bridge, AppDialogs, StorageBreakdownTable, StorageCleanupActions, StorageManagementModal, and CaCertificateInstallModal; the host critical gate includes `proxyCertificateService.ts` at 85% lines/75% branches and the file reports 100%/100%; shared runtime ensure/start/stop boundaries have direct application tests, the proxy insight formatter has deterministic output contracts, API/JSONL ingress plus WebSocket transport have direct routing, startup-failure, timeout, and reconnection contracts, proxy state persistence has direct atomicity, concurrency, cleanup, mode, malformed-input, and failure-path tests, token-detector output adds pure-formatting and OutputChannel lifecycle coverage, traffic-summary construction adds direct decode, error, correlation, and immutability contracts, MITM startup now has direct single-flight, listener-failure cleanup, retry, and staged-initialization contracts, proxy log lifecycle/file-reader/entry-processor/storage contracts cover partial lines, request/response duration, malformed/error records, inactive generations, deterministic discovery, offsets, truncation, missing files, serialized writes, rotation, and retention, agent usage/relationship extraction and conversation-context parsing have direct policy contracts, RunSSE response handling has direct callback, decoder-cleanup, and initialization-race contracts, profile import decisions, profile import/export command adapters, profile CRUD command adapters, accounts-panel launch workflow adapters, accounts-panel efficiency/suggestion/explorer adapters, AccountsPanelDataRefresher, profile proxy lifecycle, certificate material generation, status-bar display-policy decisions, binary decoder payload/recovery adapters, shared header-redaction policy, separated traffic-summary/UI-line-formatting boundaries, and proxy-settings application/restoration/backup boundaries have direct cancellation, validation, dialog, serialization, deduplication, routing, error, inactive-webview, result-message, concurrent-ensure, and presentation-state coverage, and ProfileCard/App bridge/dialog/storage/certificate tests cover enterprise, error, privacy, avatar, workspace, efficiency, menu, export, initialization fallback, certificate installation, certificate failures, storage breakdown, cleanup restrictions, confirmation, progress, and listener-lifecycle behavior; all previously recorded Composer, efficiency, storage, certificate, proxy-runtime, MITM, insight, ProxyManager, extension, webview, protobuf, Accounts panel, diagnostics, streaming decoder/framing, profile-stop, token-resolution, RunSSE, Agent Tracking, quota-refresh, profile-import, process-parser, command-tokenizer, certificate-directory, shared-proto-runtime, shared-proto-runtime audit registration, sensitive-redaction, ProfileWorkspaceService, accounts-panel proxy-state, GitHub enrichment, workspace-state, and request-logger rotation suites remain part of the full-audit evidence; Repowise ingests the merged host and webview LCOV reports with 374 retained files at 90.2% lines and 82.2% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | cc1f4fb |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 424 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `cc1f4fb`
...
extraction, and proxy-settings application/restoration/backup responsibility
separation.
The complete audit included:

- extension-host tests: passed across the compiled host test suite;
- 1,193 host tests and coverage of 89.78% lines, 82.79% branches, and 85.97%
  functions;
- architecture rules and fixtures: passed for 424 production TypeScript files;
...
the code tree at `cc1f4fb` using `--code-only` and no clustering. It scanned
871 code files and contains 5,213 nodes and 15,485 raw edges; the directed
post-build diagnostic graph contains 13,491 edges.
The highest relevant
hotspots include:

- `t()`: degree 120;
- ProxyManager: degree 41 in the god-node ranking; the facade remains an
...
directed multigraph diagnostic reports 1,836 dangling endpoint edges, one
self-loop, no missing endpoints, and 157 same-endpoint relation groups; it
retains 13,649 valid candidate edges and collapses them to 13,491 directed
...
The latest Repowise index covers the code tree after `cc1f4fb`; the host and
...
375 file entries (342 exact and 33 resolved mappings); the retained coverage
index reports 374 files, 90.2% lines, and 82.2% branches.
Detailed health reports average health at 8.93/10, hotspot health at 6.82/10,
...
`src/services/proxySettingsService.ts` (4.95/10, NLOC 38, max CCN 1, max
nesting 0, 84.91% lines and 100.00% branches; remaining findings are
compatibility-facade cohesion/duplication, coverage-gradient, and historical
churn/entropy signals). The extracted application boundaries report
`proxySettingsApplicationService.ts` at 9.29/10 (NLOC 66, max CCN 3, max
nesting 3, 94.67% lines and 85.71% branches),
