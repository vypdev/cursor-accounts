...
| QA-8 | Local test confidence | PARTIAL | Full audit at `729d69c` passes 1,250 host tests and 74 webview tests across 18 files; exact host coverage is 90.94% lines/84.18% branches/87.08% functions and webview coverage is 95.39% lines/89.53% branches/94.15% functions; architecture is checked for 430 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, typed ProxyManager construction/branch behavior, session-token capture/accounting/report rendering, and webview message-state family boundaries have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 388 files at 91.36% lines and 84.06% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | 729d69c |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `729d69c`
...
- webview tests: 18 files and 74 tests passed, with 95.39% lines, 89.53%
  branches, and 94.15% functions under the V8 coverage gate;
...
tree at `729d69c`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 894 code files and
contains 5,344 nodes and 15,137 raw edges; the normalized post-build
diagnostic graph contains 12,921 edges.
...
directed multigraph diagnostic reports 2,057 dangling endpoint edges, one
self-loop, no missing endpoints, and 158 same-endpoint relation groups; it
retains 13,080 valid candidate edges and collapses them to 12,921 normalized
...
The latest Repowise index covers the code tree after `729d69c`; the host and
...
389 file entries (348 exact and 41 resolved mappings); the retained coverage
index reports 388 files, 91.36% lines, and 84.06% branches. The latest stored
global health snapshot reports average health at 8.97/10, hotspot health at
