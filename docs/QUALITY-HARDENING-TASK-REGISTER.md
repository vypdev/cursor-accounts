...
| QA-8 | Local test confidence | PARTIAL | Full audit at `792fd65` passes 1,257 host tests and 77 webview tests across 18 files; exact host coverage is 91.00% lines/84.31% branches/87.15% functions and webview coverage is 96.13% lines/90.64% branches/94.38% functions; architecture is checked for 431 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, typed ProxyManager construction/branch behavior, session-token capture/accounting/report rendering, webview message-state family boundaries, ProfileCard identity/actions behavior, efficiency database lifecycle/recovery, and proxy traffic usage coordination have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 391 files at 91.48% lines and 84.19% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | 792fd65 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 431 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `792fd65`
...
- 1,257 host tests and exact coverage of 91.00% lines, 84.31% branches, and
  87.15% functions;
- architecture rules and fixtures: passed for 431 production TypeScript files;
...
tree at `792fd65`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 898 code files and
contains 5,363 nodes and 15,204 raw edges; the normalized post-build
diagnostic graph contains 12,976 edges.
...
directed multigraph diagnostic reports 2,069 dangling endpoint edges, one
self-loop, no missing endpoints, and 158 same-endpoint relation groups; it
retains 13,135 valid candidate edges and collapses them to 12,976 normalized
...
The latest Repowise index covers the code tree after `792fd65`; the host and
...
392 file entries (349 exact and 43 resolved mappings); the retained coverage
index reports 391 files, 91.48% lines, and 84.19% branches. The latest health
...
performance scan reports 98 findings at 12.82 per 10K covered LOC. These findings are
