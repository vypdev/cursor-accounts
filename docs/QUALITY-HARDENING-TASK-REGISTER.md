...
| Latest implementation commit | 2967be4 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `2967be4`
...
tree at `2967be4`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 900 code files and
contains 5,387 nodes and 15,271 raw edges; the normalized post-build
diagnostic graph contains 13,034 edges.
...
Graphify skipped non-code files by contract, ten SQL contributions because
`tree_sitter_sql` is not installed, and `.npmrc` as potentially sensitive. The
directed multigraph diagnostic reports 2,078 dangling endpoint edges, one
self-loop, no missing endpoints, and 158 same-endpoint relation groups; it
retains 13,193 valid candidate edges and collapses them to 13,034 normalized
...
The latest Repowise index covers the code tree after `2967be4`; the host and
...
snapshot reports average health at 8.97/10, hotspot health at 6.97/10, and
