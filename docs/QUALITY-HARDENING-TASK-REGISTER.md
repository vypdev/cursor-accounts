...
| Latest implementation commit | 0f96c1b |
...
The latest `pnpm run audit` passed on 2026-08-27 at commit `0f96c1b` with:
...
`0f96c1b` using `--code-only`, no clustering, and a single worker. It scanned
822 code files and contains 4,953 nodes and 13,888 raw edges; the directed
post-build diagnostic graph contains 11,839 edges.
...
directed multigraph diagnostic reports 1,906 dangling endpoint edges, one
self-loop, no missing endpoints, and 142 same-endpoint relation groups; it
retains 11,982 valid candidate edges and collapses them to 11,839 directed
...
The latest Repowise index covers the code tree after `0f96c1b`; the host and
...
345 file entries (312 exact and 33 resolved mappings); the retained coverage
index reports 344 files, 87.58% lines, and 80.38% branches.
Detailed health reports average health at 8.85/10, hotspot health at 6.68/10,
worst-performer health at 3.51/10 for `src/ui/accountsPanelDataRefresher.ts`,
maintainability at 9.51/10 overall with a 9.07/10 hotspot score, and
