...
|---|---|---|---|---|
| QA-0 | Baseline and finding reclassification | COMPLETE | `pnpm run audit` passes at `cd213ff`; current Graphify/Repowise/dependency evidence is captured on 2026-08-27 for the certificate-operations boundary slice | Keep this baseline immutable and update it after every cross-cutting change |
...
| Latest implementation commit | cd213ff |
...
The latest `pnpm run audit` passed on 2026-08-27 at commit `cd213ff` with:
...
`cd213ff` using `--code-only`, no clustering, and a single worker. It contains
785 code files, 6,167 nodes, and 15,103 raw edges; the post-build diagnostic
graph contains 13,091 edges.
...
multigraph diagnostic reports 1,879 dangling endpoint edges, one self-loop,
...
The latest Repowise index covers the code tree after `cd213ff`; the host and
...
index reports 324 files, 87.1% lines, and 80.1% branches.
Detailed health reports average health at 8.83/10, hotspot health at 6.59/10,
