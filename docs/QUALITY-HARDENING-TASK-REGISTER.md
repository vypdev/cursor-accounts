...
| Latest implementation commit | abb66cc |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `abb66cc`
...
tree at `abb66cc`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 910 code files and
contains 5,457 nodes and 15,450 raw edges; the normalized post-build
diagnostic graph contains 13,197 directed edges.
...
directed multigraph diagnostic reports 2,088 dangling endpoint edges, one
self-loop, no missing endpoints, and 164 same-endpoint relation groups; it
retains 13,362 valid candidate edges and collapses them to 13,197 normalized
...
The latest Repowise index covers the code tree after `abb66cc`; the host and
...
index reports 395 files, 91.9% lines, and 84.6% branches. The latest health
...
performance scan reports 97 findings at 12.54 per 10K covered LOC. These findings are
