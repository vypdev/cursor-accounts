...
| QA-8 | Local test confidence | PARTIAL | Full audit at `d178509` passes 1,250 host tests and 77 webview tests across 18 files; exact host coverage is 90.94% lines/84.18% branches/87.08% functions and webview coverage is 96.13% lines/90.64% branches/94.38% functions; architecture is checked for 430 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, typed ProxyManager construction/branch behavior, session-token capture/accounting/report rendering, webview message-state family boundaries, and ProfileCard identity/actions behavior have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 390 files at 91.44% lines and 84.15% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | d178509 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 430 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `d178509`
...
- 1,250 host tests and exact coverage of 90.94% lines, 84.18% branches, and
  87.08% functions;
- architecture rules and fixtures: passed for 430 production TypeScript files;
...
The session-token accounting contract suite passed 3/3 tests, including
...
tree at `d178509`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 896 code files and
contains 5,350 nodes and 15,164 raw edges; the normalized post-build
diagnostic graph contains 12,942 edges.
...
directed multigraph diagnostic reports 2,063 dangling endpoint edges, one
self-loop, no missing endpoints, and 158 same-endpoint relation groups; it
retains 13,101 valid candidate edges and collapses them to 12,942 normalized
...
- Historical security scan: 0 findings across 214 commits, 4,539 blobs, and
  2,578 files.
- Current refactoring targets include `scripts/analyze-proxy-traffic.mjs`,
  `webview/src/appMessageState.ts`, `webview/src/api/vscodeApi.ts`,
  `src/commands/proxyCommands.ts`, `webview/src/components/ProfileCard.tsx`,
  `src/persistence/efficiencyDatabase.ts`, `src/ui/accountsPanel.ts`,
  `src/proxy/mitmProxyResponseHandler.ts`, and
  `src/profiles/profileLauncher.ts`. These are prioritization candidates only:
...
The latest Repowise index covers the code tree after `d178509`; the host and
...
391 file entries (348 exact and 43 resolved mappings); the retained coverage
index reports 390 files, 91.44% lines, and 84.15% branches. The latest stored
global health snapshot reports average health at 8.98/10, hotspot health at
6.91/10, worst-performer health at 4.15/10 for
`packages/types/src/index.ts`, maintainability at 9.56/10 overall with a
9.19/10 hotspot score, and performance at 9.95/10 overall with a 9.92/10
hotspot score. These findings are
...
Repowise reports the database facade at 5.73/10 (NLOC 272, max CCN 7, max
nesting 3, 88.35% lines and 72.97% branches); the aggregate reader at 9.52/10
