...
| QA-8 | Local test confidence | PARTIAL | Full audit at `f05bbe1` passes 1,250 host tests and 71 webview tests across 18 files; exact host coverage is 90.94% lines/84.18% branches/87.08% functions and webview coverage is 94.49% lines/85.46% branches/93.93% functions; architecture is checked for 430 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, typed ProxyManager construction/branch behavior, and session-token capture/accounting/report rendering have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 383 files at 91.1% lines and 83.3% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | f05bbe1 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `f05bbe1`
...
- webview tests: 18 files and 71 tests passed, with 94.49% lines, 85.46%
  branches, and 93.93% functions under the V8 coverage gate;
...
tree at `f05bbe1`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 889 code files and
contains 5,326 nodes and 15,078 raw edges; the normalized post-build
diagnostic graph contains 12,872 edges.
...
directed multigraph diagnostic reports 2,052 dangling endpoint edges, one
self-loop, no missing endpoints, and 152 same-endpoint relation groups; it
retains 13,026 valid candidate edges and collapses them to 12,872 normalized
...
The latest Repowise index covers the code tree after `f05bbe1`; the host and
...
384 file entries (348 exact and 36 resolved mappings); the retained coverage
index reports 383 files, 91.1% lines, and 83.3% branches. The latest stored
global health snapshot reports average health at 8.95/10, hotspot health at
...
historical co-change signals. The current webview state reducer is a concrete
candidate at CCN 34, maximum nesting 5, 87.29% lines, and 57.14% branches;
it requires a message-family/state-transition review before any extraction is
attempted.
