...
| Latest implementation commit | 192942a |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 400 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `192942a`
with the follow-up filesystem-error hardening at `42ed7a8`, RunSSE serialization
at `88d35fe`, response-finalization context extraction at `b241bc5`, and profile
import/export command decomposition at `192942a`. The
complete audit
included:

- extension-host tests: passed across the compiled host test suite;
- 1,161 host tests and coverage of 88.90% lines, 82.21% branches, and 85.51%
  functions;
- architecture rules and fixtures: passed for 400 production TypeScript files;
...
`192942a` using `--code-only` and no clustering. It scanned 843 code files and
contains 5,070 nodes and 14,296 raw edges; the directed post-build diagnostic
graph contains 12,176 edges.
...
directed multigraph diagnostic reports 1,973 dangling endpoint edges, one
self-loop, no missing endpoints, and 145 same-endpoint relation groups; it
retains 12,322 valid candidate edges and collapses them to 12,176 directed
...
The latest Repowise index covers the code tree after `192942a`; the host and
...
356 file entries (323 exact and 33 resolved mappings); the retained coverage
index reports 355 files, 89.4% lines, and 81.5% branches.
