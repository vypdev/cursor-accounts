...
| QA-1 | CI, tests, localization, lint | PARTIAL | Full audit passes with 891 host tests, 81.52% lines, 74.78% branches, and 78.31% functions; localization is 26 locales/428 keys and webview is 18/18 | Add repeated host-run evidence and review generated-artifact/source-lint policy |
...
| QA-5 | Clean Architecture enforcement | PARTIAL | TypeScript-AST resolver-backed checker passes for 311 production files; negative fixtures cover domain/application/package/cycle cases; storage cleanup keeps efficiency-event SQLite construction behind a domain port and persistence adapter, and policy-specific actions behind a cohesive runner; current Graphify/Repowise hotspots remain | Refactor the highest-risk low-coverage infrastructure modules without weakening the contract, then add characterization and failure-path tests |
...
| Latest implementation commit | 22bc487 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 311 production TypeScript files; tests excluded by contract |
...
- 891 host tests and coverage of 81.52% lines, 74.78% branches, and 78.31%
  functions;
...
The latest graph was generated with Graphify `0.9.48` on commit `22bc487` using
no clustering. It contains 5,406 nodes and 12,807 raw edges; 646 files were
uncached during extraction, and the post-build graph contains 11,044 edges.
...
diagnostic reports 1,568 dangling endpoint edges, one self-loop, no missing
...
The latest Repowise health report scores average health at 8.48/10, hotspot
health at 5.74/10, and performance at 9.91/10. The extracted insight facade
