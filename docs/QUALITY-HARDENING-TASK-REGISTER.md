...
| Latest implementation commit | 1e0d366 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `1e0d366`
...
explicit settings restoration and modeled runtime actions.
The complete audit included:

- extension-host tests: passed across the compiled host test suite;
- 1,227 host tests and exact coverage of 90.56% lines, 83.58% branches, and
  86.50% functions;
...
copy of the code tree at `1e0d366` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 881 code
files and contains 5,289 nodes and 15,731 raw edges; the normalized post-build
diagnostic graph contains 13,718 edges.
...
directed multigraph diagnostic reports 1,855 dangling endpoint edges, one
self-loop, no missing endpoints, and 157 same-endpoint relation groups; it
retains 13,876 valid candidate edges and collapses them to 13,718 normalized
...
The latest Repowise index covers the code tree after `1e0d366`; the host and
