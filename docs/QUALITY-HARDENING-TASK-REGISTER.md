...
| QA-8 | Local test confidence | PARTIAL | Full audit at `f03e423` passes 1,241 host tests and 70 webview tests across 18 files; exact host coverage is 90.73% lines/83.80% branches/86.78% functions and webview coverage is 94.19% lines/85.40% branches/92.73% functions; architecture is checked for 430 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, and protobuf analysis CLI boundaries have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered by integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 383 files at 90.9% lines and 83.2% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | f03e423 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `f03e423`
...
- webview tests: 18 files and 70 tests passed, with 94.19% lines, 85.40%
  branches, and 92.73% functions under the V8 coverage gate;
...
copy of the code tree at `f03e423` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 882 code
files and contains 5,299 nodes and 15,760 raw edges; the normalized post-build
diagnostic graph contains 13,738 edges.
...
retains 13,896 valid candidate edges and collapses them to 13,738 normalized
...
The latest Repowise index covers the code tree after `f03e423`; the host and
...
index reports 383 files, 90.9% lines, and 83.2% branches.
