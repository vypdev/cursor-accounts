...
| Latest implementation commit | ae74593 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 418 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `ae74593`
...
failures, status-bar display-policy extraction, and binary decoder recovery-path
isolation and coverage, followed by the traffic-summary/domain and
UI/presentation boundary extraction.
...
- 1,193 host tests and coverage of 89.78% lines, 82.79% branches, and 85.97%
  functions;
- architecture rules and fixtures: passed for 418 production TypeScript files;
...
`ae74593` using `--code-only` and no clustering. It scanned 865 code files and
contains 5,174 nodes and 14,624 raw edges; the directed post-build diagnostic
graph contains 12,465 edges.
...
directed multigraph diagnostic reports 2,007 dangling endpoint edges, one
self-loop, no missing endpoints, and 150 same-endpoint relation groups; it
retains 12,616 valid candidate edges and collapses them to 12,465 directed
...
- Safe-only dead-code analysis: no findings. The current index reports zero
  unreachable files and zero unused exports; the unrestricted heuristic report
...
The latest Repowise index covers the code tree after `ae74593`; the host and
...
371 file entries (338 exact and 33 resolved mappings); the retained coverage
index reports 370 files, 90.2% lines, and 82.2% branches.
Detailed health reports average health at 8.93/10, hotspot health at 6.78/10,
...
`src/services/proxySettingsService.ts` (5.38/10, NLOC 123, max CCN 6, max
nesting 4, 87.67% lines and 77.27% branches; remaining findings are low
cohesion, nested complexity, coverage-gradient, and historical churn signals),
