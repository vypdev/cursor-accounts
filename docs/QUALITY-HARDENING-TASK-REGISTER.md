...
| Latest implementation commit | fa38bef |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `fa38bef`
...
and remaining Accounts panel presentation-workflow extraction.
The
complete audit
included:

- extension-host tests: passed across the compiled host test suite;
- 1,173 host tests and coverage of 89.46% lines, 82.44% branches, and 85.57%
  functions;
...
`fa38bef` using `--code-only` and no clustering. It scanned 853 code files and
contains 5,115 nodes and 14,454 raw edges; the directed post-build diagnostic
graph contains 12,315 edges.
...
retains 12,461 valid candidate edges and collapses them to 12,315 directed
...
The latest Repowise index covers the code tree after `fa38bef`; the host and
...
index reports 362 files, 89.9% lines, and 81.9% branches.
