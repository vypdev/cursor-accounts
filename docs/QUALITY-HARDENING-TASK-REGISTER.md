...
| Latest implementation commit | 63411e0 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 392 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest `pnpm run audit` passed on 2026-08-27 at commit `63411e0` with:
...
- 1,112 host tests and coverage of 86.85% lines, 81.02% branches, and 84.56%
  functions;
- architecture rules and fixtures: passed for 392 production TypeScript files;
...
`63411e0` using `--code-only`, no clustering, and a single worker. It scanned
827 code files and contains 4,976 nodes and 13,955 raw edges; the directed
post-build diagnostic graph contains 11,888 edges.
...
- scripts: degree 70;
- IProfileDetector: degree 54;
- IProfileReader: degree 62;
...
directed multigraph diagnostic reports 1,924 dangling endpoint edges, one
self-loop, no missing endpoints, and 142 same-endpoint relation groups; it
retains 12,031 valid candidate edges and collapses them to 11,888 directed
...
The latest Repowise index covers the code tree after `63411e0`; the host and
...
346 file entries (313 exact and 33 resolved mappings); the retained coverage
index reports 345 files, 87.82% lines, and 80.67% branches.
Detailed health reports average health at 8.86/10, hotspot health at 6.71/10,
worst-performer health at 3.71/10 for `src/ui/accountsPanelDataRefresher.ts`,
maintainability at 9.51/10 overall with a 9.06/10 hotspot score, and
...
Repowise sync metadata points to commit `63411e0`, with no embedding provider or
