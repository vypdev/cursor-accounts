...
| Latest implementation commit | 3ef9ecd |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `3ef9ecd`
...
- 1,242 host tests and exact coverage of 90.83% lines, 84.08% branches, and
  86.78% functions;
...
copy of the code tree at `3ef9ecd` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 882 code
files and contains 5,302 nodes and 15,772 raw edges; the normalized post-build
diagnostic graph contains 13,747 edges.
...
retains 13,905 valid candidate edges and collapses them to 13,747 normalized
...
- Current refactoring targets include `scripts/verify-proto-jsonl.mjs`,
  ...
  `src/ui/accountsPanelDataRefresher.ts`, `src/proxy/requestLogger.ts`, and
...
The latest Repowise index covers the code tree after `3ef9ecd`; the host and
...
The latest stored global health snapshot reports average health at 8.96/10,
hotspot health at 6.90/10,
