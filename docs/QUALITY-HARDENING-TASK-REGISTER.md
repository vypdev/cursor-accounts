...
| QA-8 | Local test confidence | PARTIAL | Full audit at `616376e` passes 1,257 host tests and 77 webview tests across 18 files; exact host coverage is 91.03% lines/84.40% branches/87.16% functions and webview coverage is 96.13% lines/90.64% branches/94.38% functions; architecture is checked for 432 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, protobuf descriptor extraction, protobuf JSONL route/evaluation, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, typed ProxyManager construction/branch behavior, session-token capture/accounting/report rendering, webview message-state family boundaries, ProfileCard identity/actions behavior, efficiency database lifecycle/recovery, proxy traffic usage coordination, profile import feedback, agent extraction, and proxy traffic analysis have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 392 files at 91.5% lines and 84.4% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | 616376e |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 432 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `616376e`
...
tree at `616376e`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 906 code files and
contains 5,425 nodes and 15,377 raw edges; the normalized post-build
diagnostic graph contains 13,130 directed edges.
...
directed multigraph diagnostic reports 2,082 dangling endpoint edges, one
self-loop, no missing endpoints, and 164 same-endpoint relation groups; it
retains 13,295 valid candidate edges and collapses them to 13,130 normalized
...
The latest Repowise index covers the code tree after `616376e`; the host and
...
393 file entries (350 exact and 43 resolved mappings); the retained coverage
index reports 392 files, 91.5% lines, and 84.4% branches. The latest health
snapshot reports average health at 9.00/10, hotspot health at 6.92/10, and
