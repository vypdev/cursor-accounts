...
| Latest implementation commit | ed9eb9e |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 435 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-28 at commit `ed9eb9e`
...
- 1,275 host tests and exact coverage of 91.03% lines, 84.40% branches, and
  87.16% functions;
- architecture rules and fixtures: passed for 432 production TypeScript files;
...
tree at `ed9eb9e`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 911 code files and
contains 5,461 nodes and 15,463 raw edges; the normalized post-build
diagnostic graph contains 13,205 directed edges.
...
directed multigraph diagnostic reports 2,093 dangling endpoint edges, one
self-loop, no missing endpoints, and 164 same-endpoint relation groups; it
retains 13,370 valid candidate edges and collapses them to 13,205 normalized
...
The latest Repowise index covers the code tree after `ed9eb9e` and reports
5,669 nodes with 15,402 edges; the host and webview lcov reports were
explicitly merged with `repowise coverage add
coverage/lcov.info webview/coverage/lcov.info`. The latest ingestion accepted
396 file entries (353 exact and 43 resolved mappings); the retained coverage
index reports 395 files, 92.0% lines, and 84.7% branches. The latest health
snapshot reports average health at 9.00/10, hotspot health at 6.91/10, and
worst-performer health at 4.15/10 for `packages/types/src/index.ts`; the
performance scan reports 97 findings at 12.52 per 10K covered LOC. These findings are
