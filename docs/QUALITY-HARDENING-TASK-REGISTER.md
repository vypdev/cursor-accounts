...
| Latest implementation commit | 8d8a225 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 394 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `8d8a225`
...
- 1,156 host tests and coverage of 88.33% lines, 82.19% branches, and 85.34%
  functions;
...
`8d8a225` using `--code-only` and no clustering. It scanned 838 code files and
contains 5,041 nodes and 14,183 raw edges; the directed post-build diagnostic
graph contains 12,076 edges.
...
directed multigraph diagnostic reports 1,964 dangling endpoint edges, one
self-loop, no missing endpoints, and 142 same-endpoint relation groups; it
retains 12,219 valid candidate edges and collapses them to 12,076 directed
...
The latest Repowise index covers the code tree after `8d8a225`; the host and
...
index reports 353 files, 88.9% lines, and 81.5% branches.
...
Repowise sync metadata points to commit `8d8a225`, with no embedding provider or
