...
| QA-1 | CI, tests, localization, lint | PARTIAL | Full audit passes with 877 host tests, 81.08% lines, 74.94% branches, and 77.94% functions; localization is 26 locales/428 keys and webview is 18/18 | Add repeated host-run evidence and review generated-artifact/source-lint policy |
...
| Latest implementation commit | e3a4f9f |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 306 production TypeScript files; tests excluded by contract |
...
- 877 host tests and coverage of 81.08% lines, 74.94% branches, and 77.94%
  functions;
- architecture rules and fixtures: passed for 306 production TypeScript files;
...
The latest graph was generated with Graphify `0.9.48` on commit `e3a4f9f` using
no clustering. It contains 5,350 nodes and 12,645 raw edges; 636 files were
uncached during extraction, and the post-build graph contains 10,907 edges.
...
diagnostic reports 1,544 dangling endpoint edges, one self-loop, no missing
...
health at 5.68/10, and performance at 9.90/10. The extracted insight facade
...
current production targets include `src/proxy/mitmProxyServer.ts` (4.5/10,
NLOC 214, max CCN 7),
...
The new `src/proxy/mitmProxyErrorHandler.ts` scores 9.0/10 with NLOC 43,
max CCN 9, nesting 1, and a dedicated test. Its remaining static markers are
limited to one complex factory method and one duplication signal. The server
startup method remains the next proxy hotspot and is intentionally retained as
the composition root until its lifecycle seams have characterization and
