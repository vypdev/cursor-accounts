...
|---|---|---|---|---|
| QA-0 | Baseline and finding reclassification | COMPLETE | pnpm run audit passes; current Graphify/Repowise/dependency evidence captured on 2026-08-26 at `be3d831` | Keep this baseline immutable and update it after every cross-cutting change |
| QA-1 | CI, tests, localization, lint | PARTIAL | Full audit passes with 923 host tests, 82.39% lines, 75.51% branches, and 79.90% functions; localization is 26 locales/428 keys and webview is 30/30 across 7 files | Add repeated host-run evidence and review generated-artifact/source-lint policy |
...
| Latest implementation commit | be3d831 |
...
The latest `pnpm run audit` passed on 2026-08-26 at commit `be3d831` with:
...
- webview tests: 7 files and 30 tests passed;
...
The latest graph was generated with Graphify `0.9.48` on commit `be3d831` using
no clustering. It contains 5,531 nodes and 13,247 raw edges; 666 files were
uncached during extraction, and the post-build graph contains 11,424 edges.
...
- Quality Hardening Task Register: degree 50;
...
diagnostic reports 1,621 dangling endpoint edges, one self-loop, no missing
...
The latest Repowise index is synchronized to commit `be3d831`; the current
lcov report was then explicitly ingested with `repowise coverage add
coverage/lcov.info`. Detailed health reports average health at 8.57/10,
hotspot health at 5.95/10, worst-file health at 3.11/10 for `src/extension.ts`,
maintainability at 9.43/10, and performance at 9.92/10. The detailed report
...
`webview/src/App.tsx` (4.93/10, NLOC 572, max CCN 11, no direct test file) and
...
Repowise sync metadata points to commit `be3d831`, with no embedding provider or
...
|---|---|---|
| Webview DOM leakage | PARTIALLY RESOLVED | Current webview suite passes 30/30; repeated and shuffled-run evidence remains |
