...
| QA-8 | Local test confidence | PARTIAL | Full audit at `895da24` passes 1,215 host tests and 70 webview tests across 18 files; exact host coverage is 90.37% lines/83.20% branches/86.43% functions and webview coverage is 94.19% lines/85.40% branches/92.73% functions; architecture is checked for 426 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup and VS Code window-configuration boundaries have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered by integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 379 files at 90.72% lines and 82.94% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | 895da24 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 426 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `895da24`
...
relationship enrichment, and extension window initialization coverage.
The complete audit included:

- extension-host tests: passed across the compiled host test suite;
- 1,215 host tests and exact coverage of 90.37% lines, 83.20% branches, and
  86.43% functions;
- architecture rules and fixtures: passed for 426 production TypeScript files;
...
copy of the code tree at `895da24` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 876 code
files and contains 5,262 nodes and 15,645 raw edges; the normalized post-build
diagnostic graph contains 13,635 edges.
...
directed multigraph diagnostic reports 1,848 dangling endpoint edges, one
self-loop, no missing endpoints, and 156 same-endpoint relation groups; it
retains 13,797 valid candidate edges and collapses them to 13,635 normalized
...
- Safe-only dead-code analysis: no findings. The current index reports zero
  unreachable files and one heuristic unused-export signal; the unrestricted report
...
The latest Repowise index covers the code tree after `895da24`; the host and
...
380 file entries (344 exact and 36 resolved mappings); the retained coverage
index reports 379 files, 90.54% lines, and 82.86% branches.
Detailed health reports average health at 8.95/10, hotspot health at 6.81/10,
