...
| Latest implementation commit | 97e230a |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `97e230a`
...
`EfficiencyAnalyzer` facade coverage for queue and error contracts.
...
The latest graph was generated with Graphify `0.9.48` from a clean temporary
copy of the code tree at `97e230a` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 883 code
files and contains 5,306 nodes and 15,776 raw edges; the normalized post-build
diagnostic graph contains 13,751 edges.
...
- IProfileReader: degree 71;
- IProfileDetector: degree 69;
- ProxyManager: degree 41 in the god-node ranking; it remains an intentional
  public coordination boundary after event/diagnostics extraction;
- scripts: degree 70;
...
directed multigraph diagnostic reports 1,867 dangling endpoint edges, one
self-loop, no missing endpoints, and 157 same-endpoint relation groups; it
retains 13,909 valid candidate edges and collapses them to 13,751 normalized
...
The latest Repowise index covers the code tree after `97e230a`; the host and
...
index reports 383 files, 91.1% lines, and 83.3% branches.
The latest stored global health snapshot reports average health at 8.95/10,
hotspot health at 6.89/10,
worst-performer health at 4.15/10 for `packages/types/src/index.ts`,
maintainability at 9.54/10 overall with a 9.12/10 hotspot score, and
