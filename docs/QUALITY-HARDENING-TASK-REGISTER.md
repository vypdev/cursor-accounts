...
| Latest implementation commit | e936804 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `e936804`
...
- 1,241 host tests and exact coverage of 90.73% lines, 83.80% branches, and
  86.78% functions;
...
copy of the code tree at `e936804` with documentation, generated output, and
dependency directories excluded, using no clustering. It scanned 882 code
files and contains 5,299 nodes and 15,761 raw edges; the normalized post-build
diagnostic graph contains 13,739 edges.
...
directed multigraph diagnostic reports 1,864 dangling endpoint edges, one
self-loop, no missing endpoints, and 157 same-endpoint relation groups; it
retains 13,897 valid candidate edges and collapses them to 13,739 normalized
...
The latest Repowise index covers the code tree after `e936804`; the host and
...
`src/ui/accountsPanelHandlers.ts` (5.36/10, NLOC 211, max CCN 2, max nesting
1, 86.53% lines and 54.29% branches), while the extracted
