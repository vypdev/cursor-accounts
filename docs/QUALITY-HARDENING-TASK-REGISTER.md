...
| Latest implementation commit | 1451171 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `1451171`
...
`1451171` using `--code-only` and no clustering. It scanned 858 code files and
contains 5,143 nodes and 14,526 raw edges; the directed post-build diagnostic
graph contains 12,379 edges.
...
retains 12,524 valid candidate edges and collapses them to 12,379 directed
...
The latest Repowise index covers the code tree after `1451171`; the host and
...
index reports 365 files, 90.06% lines, and 82.08% branches.
...
`src/proxy/certificateManager.ts` (4.36/10, NLOC 79, max CCN 4, max nesting 4,
82.52% lines and 90.91% branches; the remaining signal is lifecycle/error-path
