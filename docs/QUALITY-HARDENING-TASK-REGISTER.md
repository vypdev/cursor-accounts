...
| Latest implementation commit | a6b1f16 |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `a6b1f16`
...
- 1,149 host tests and coverage of 88.10% lines, 81.77% branches, and 85.31%
  functions;
- architecture rules and fixtures: passed for 396 production TypeScript files;
...
`a6b1f16` using `--code-only` and no clustering. It scanned 836 code files and
contains 5,033 nodes and 14,167 raw edges; the directed post-build diagnostic
graph contains 12,065 edges.
...
directed multigraph diagnostic reports 1,959 dangling endpoint edges, one
self-loop, no missing endpoints, and 142 same-endpoint relation groups; it
retains 12,208 valid candidate edges and collapses them to 12,065 directed
...
The latest Repowise index covers the code tree after `a6b1f16`; the host and
...
350 file entries (317 exact and 33 resolved mappings); the retained coverage
index reports 349 files, 88.65% lines, and 81.34% branches.
Detailed health reports average health at 8.87/10, hotspot health at 6.71/10,
...
performance at 9.94/10 overall with a 9.86/10 hotspot score. These findings are
...
failure isolation. `src/proxy/requestLogger.ts` scores 5.29/10 at NLOC 189
with 77.17% lines and 77.78% branches; the latest direct suite covers six
contracts, including the previously untested rotation queue deadlock. Its
remaining low-cohesion, coverage-gradient, and filesystem-loop signals are
retained for a later adapter split only if runtime behavior and ownership make
that extraction worthwhile.
...
Repowise sync metadata points to commit `a6b1f16`, with no embedding provider or
