...
| Latest implementation commit | 1a475fb |
...
The latest complete `pnpm run audit` passed on 2026-08-27 at commit `1a475fb`
...
certificate material read failures.
The
complete audit
included:

- extension-host tests: passed across the compiled host test suite;
- 1,193 host tests and coverage of 89.65% lines, 82.62% branches, and 85.82%
  functions;
- architecture rules and fixtures: passed for 412 production TypeScript files;
...
`1a475fb` using `--code-only` and no clustering. It scanned 858 code files and
contains 5,144 nodes and 14,528 raw edges; the directed post-build diagnostic
graph contains 12,381 edges.
...
directed multigraph diagnostic reports 2,002 dangling endpoint edges, one
self-loop, no missing endpoints, and 144 same-endpoint relation groups; it
retains 12,526 valid candidate edges and collapses them to 12,381 directed
...
The latest Repowise index covers the code tree after `1a475fb`; the host and
...
index reports 365 files, 90.07% lines, and 82.10% branches.
Detailed health reports average health at 8.92/10, hotspot health at 6.78/10,
worst-performer health at 4.15/10 for `packages/types/src/index.ts`,
maintainability at 9.53/10 overall with a 9.11/10 hotspot score, and
