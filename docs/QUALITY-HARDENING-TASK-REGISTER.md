...
|---|---|---|---|---|
| QA-0 | Baseline and finding reclassification | COMPLETE | `pnpm run audit` passes at `de7943a`; current Graphify/Repowise/dependency evidence is captured on 2026-08-28 after the shared-proto-runtime audit-gate, workspace application boundary, GitHub/workspace/accounts-panel composition boundaries, request-logger reliability fixes and storage extraction, profile-account fetch consolidation, transactional MITM startup/composition hardening, proxy log tailer boundary extraction, agent insight policy extraction, conversation-context policy extraction, RunSSE response processing serialization, profile import/export command decomposition, profile CRUD command decomposition with failure-path coverage, accounts-panel launch workflow extraction, remaining Accounts panel presentation-workflow extraction, AccountsPanelDataRefresher coverage hardening, active-conversation totals loading isolation, profile proxy lifecycle decision isolation, certificate material generation/single-flight hardening, certificate filesystem-error propagation, certificate recovery-path coverage, status-bar display-policy extraction/refinement, binary proxy decoder/recovery-path isolation, shared proxy-header redaction, traffic-summary/presentation boundary separation, proxy-settings application-operation deduplication, proxy-settings application/restoration/backup responsibility separation, proxy-window adapter coverage, proxy-settings compatibility-facade removal, efficiency persistence read-adapter separation, efficiency database lifecycle/recovery boundary and direct contracts, agent stream payload decoding consolidation, webview profile-action bridge/form separation, shared proxy relationship enrichment, extension window initialization coverage, active-conversation status presentation extraction, profile proxy edit persistence/lifecycle separation with explicit settings restoration, direct `EfficiencyAnalyzer` facade coverage including unknown-error fallback behavior, QuotaClient fallback/merge coverage, protobuf analysis CLI executable-entrypoint coverage, webview diagnostic-bridge coverage, complete Accounts panel dispatcher coverage, Accounts panel lifecycle coverage, typed ProxyManager construction and branch-contract hardening, session-token accounting/report-rendering/capture-analysis boundaries, webview message-state family boundaries, proxy traffic usage coordination boundary, profile import presentation policy, protobuf descriptor extraction responsibility boundaries, protobuf JSONL routing/evaluation boundaries, agent stream framing/text-bucket/token-metric boundaries, proxy-entry decoder/insight/orchestration boundaries, Accounts panel proxy action boundaries, direct proxy-output presenter coverage, proxy runtime composition coverage, agent tracking repository-pool concurrency coverage, efficiency output-presentation policy/adaptor coverage, and profile launch presentation/command coverage | Keep this baseline immutable and update it after every cross-cutting change |
| QA-1 | CI, tests, localization, lint | PARTIAL | Full audit at `de7943a` passes with 1,300 host tests and 77 webview tests; exact host coverage is 92.09% lines/84.77% branches/88.50% functions, webview coverage is 96.13% lines/90.64% branches/94.38% functions, localization is 26 locales/428 keys, architecture is checked for 437 production TypeScript files, and direct contracts cover the proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, protobuf descriptor extraction, protobuf JSONL routing/evaluation, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, Accounts panel proxy runtime/certificate/output handlers, `ProxyOutputPresenter`, proxy runtime composition, agent tracking repository-pool concurrency/close/error paths, typed ProxyManager construction/branch behavior, five session-token capture/accounting/reporting contracts, the decomposed webview message-state families, ProfileCard identity/actions behavior, efficiency database lifecycle/recovery, proxy traffic usage coordination, profile import feedback, the three new agent extraction contracts, proxy traffic analysis, the pure efficiency output-presentation policy and VS Code adapter, and the pure profile launch presentation policy and launch command. The complete host suite also passes independently under the declared Node 24.19.0 runtime | Add repeated host/webview-run evidence and review generated-artifact/source-lint policy |
| QA-2 | Dependencies and supply chain | PARTIAL | SDK `1.0.28`, legacy npm `sqlite3` removed, targeted `uuid@11.1.1` and `undici@6.28.0` overrides resolve; pinned pnpm 10.34 production audit reports 0 advisories; isolated pnpm 11.19 signature audit verifies 163/163 packages; CycloneDX 1.5 SBOM contains 163 components; clean VSIX tree has no package-manager store; `audit:supply-chain` now automates the checks and CI/release/hotfix upload the report for 14 days; the full audit at `25f2f1c` passes the supply-chain stage | Review Cursor vendor terms and semaphore MIT attribution, decide release SBOM retention, and close the final attribution policy before marking complete |
...
| QA-5 | Clean Architecture enforcement | PARTIAL | TypeScript-AST resolver-backed checker passes for 437 production files; negative fixtures cover domain/application/package/cycle cases; storage cleanup keeps efficiency-event SQLite construction behind a domain port and persistence adapter, efficiency database lifecycle/recovery behind a focused persistence boundary with pre-probe sidecar snapshots and direct contracts, policy-specific actions behind a cohesive runner, proxy insight dispatch/redaction behind focused application boundaries, proxy traffic usage coordination behind an injected ingestion/logger boundary, profile import feedback behind a pure presentation policy, protobuf descriptor extraction now separates message-factory, message-reference, enum, and service responsibilities behind pure script boundaries, protobuf JSONL verification now separates route classification and pure body evaluation from report mutation, and agent analysis now separates Connect framing/decoding, recursive text-bucket extraction, and stream/prefetch metric policies behind pure script boundaries with a compatibility barrel preserving existing callers; ProxyManager construction behind a composition boundary with one typed options object, Composer polling through injected boundaries, efficiency toggling behind a dedicated workflow, extension activation through focused composition modules, build-target selection and VSIX policy behind pure/shared boundaries, webview state and bridge behavior behind typed/injectable boundaries, live usage accounting/rendering behind application/presentation policies, Accounts HTML/CSP behind a pure renderer, proxy traffic analysis and diagnostics behind focused boundaries, protobuf framing/report rendering/runtime loading behind pure script boundaries, protobuf analysis CLI loading behind explicit runtime dependencies and executable-entrypoint contracts, ProxyManager event coordination behind an application boundary, Agent Tracking ingestion behind an application use case, token resolution behind an application policy and domain OAuth port, quota refresh behind application/cache/fetch ports, persisted proxy attachment behind an application use case, streaming Connect framing/session behind adapter/application/domain boundaries, profile stop/restore behind an application use case, shared proxy startup and ensure/stop decisions behind application use cases, profile start/ensure/restart validation behind an application use case, proxy control-plane operations behind a shared domain port, traffic insight formatting behind pure helpers, API/JSONL ingress behind dedicated composition/adapters with transactional startup cleanup, WebSocket lifecycle behind a dedicated transport adapter, proxy state persistence behind a filesystem adapter with collision-safe temporary files and typed failure boundaries, token-detector formatting behind a pure presentation policy separated from the VS Code OutputChannel adapter, traffic-summary decode/correlation decisions behind a pure policy separated from summary orchestration, traffic-summary construction behind a proxy/domain boundary and live-line rendering behind a UI/presentation boundary, proxy-settings application/restoration/backup responsibilities behind dedicated application boundaries and a VS Code adapter at the composition root, ProxyManager now consumes the application and restoration contracts directly without the old compatibility facade, efficiency database aggregate/event/quota reads behind separate injected persistence adapters, ProfileCard quota/leaderboard/efficiency/workspace sections behind cohesive webview components, App host-message side effects behind a typed webview bridge hook, App message-state transitions behind typed host, proxy, storage, and pricing family modules, App dialog selection behind a presentation-only composition component, storage modal breakdown/actions behind cohesive presentation components, profile import orchestration and duplicate decisions behind explicit application helpers, profile import/export command registration behind separate VS Code adapters with focused command contracts, profile CRUD command registration behind separate adapters with cancellation and failure-path contracts, platform-specific process-output parsing behind shared pure conversion and tokenizer boundaries, certificate material, trust, MITM-directory, and operation adaptation behind explicit ports assembled at the composition root, workspace scanning behind a domain port consumed by an application service, Accounts panel initial-data and workspace-state reads behind private coordinator contracts, GitHub enrichment behind direct structural fakes, Accounts panel dependencies behind one explicit composition object, MITM startup transaction/configuration behind private composition helpers, proxy JSONL lifecycle/file-reader/entry-processing behind separate infrastructure/application boundaries, agent insight usage/relationship extraction behind cohesive feature policies, conversation-context parsing behind a focused policy, accounts-panel launch routing/deduplication behind a dedicated UI workflow adapter, Accounts panel efficiency, suggestion, and explorer actions behind focused UI adapters, certificate material generation/single-flight coordination and recovery behavior behind explicit infrastructure/application and filesystem/parsing boundaries, status-bar display-state resolution behind a pure UI policy, webview profile bridge/form actions behind composable application hooks, webview diagnostic logging behind the same typed message adapter with early-boot isolation, shared proxy relationship enrichment behind one pure policy, active-conversation status rendering behind a VS Code-free presentation policy with shared token/cost formatting, profile proxy edit persistence/runtime effects behind separate application services assembled at the composition root, the `EfficiencyAnalyzer` facade behind the existing application workflow/queue boundary with direct contracts, QuotaClient as a narrow API adapter composing typed token, HTTP, validation, and usage-mapping boundaries, the Accounts panel dispatcher routing through specialized UI adapters with a complete typed dispatch contract, the Accounts panel provider lifecycle boundary through explicit webview/workspace event contracts, and Accounts panel proxy actions through dedicated runtime, certificate, and output adapters with a compatibility facade; the efficiency output presenter now delegates all formatting, threshold, and notification decisions to a pure application policy, and profile launch choices are built by a VS Code-free presentation policy; current Graphify/Repowise hotspots remain | Split the remaining large composition functions and add direct contract tests without weakening the dependency direction |
| QA-6 | Security and privacy | PARTIAL | Threat model recorded; API error details are generic; loopback/token parity, case-insensitive protocol-header redaction including authorization, cookie, set-cookie, x-api-key, api-key, and x-auth-token, path-boundary containment, sidecar safety, text-safe webview rendering, certificate platform validation, injected certificate-process failure tests, bounded certificate-process timeout/kill escalation, fail-closed migration execution, private proxy-state file mode enforcement, SQLite snapshot backup/restore validation, sidecar preservation, partial-cleanup accounting, typed ENOSPC classification with actionable error reporting, and Repowise history scan are evidenced | Complete physical disk-full/crash-restart testing, native privileged command execution review on supported OS runners, protocol-specific redaction, and the legacy optional-token decision |
| QA-7 | Reliability and lifecycle | PARTIAL | Tracking ingress shutdown is idempotent; MITM startup is single-flight, transactional on listener failure, and bounded during close; proxy startup failures attempt MITM/ingress/API cleanup; API and JSONL ingress now clean up clients, subscriptions, and tailers when initial startup fails; the WebSocket transport bounds initial connection, prevents concurrent connect/reconnect duplication, isolates listener failures, and retries only after an established socket closes; proxy state writes use unique temporary files and clean up after rename failures; the JSONL tailer now uses lifecycle generations, single-flight startup, recoverable read serialization, stale-callback suppression, deterministic rotation, and missing/truncated-file recovery; direct `SIGTERM`/`SIGINT` uses graceful shutdown; the real runtime integration suite passes 3/3; SQLite persistence has 2-process concurrency/reopen, migration rollback/retry, consistent backup, validated restore, corrupted-database sidecar preservation, partial cleanup reporting including reclaimed-byte preservation after ENOSPC, long-lived-reader checkpoint evidence, and real child hang escalation coverage; ProxyManager event coordination now owns primary and external traffic unsubscription, listener error isolation, post-disposal registration guards, and idempotent disposal; extension deactivation awaits tracked profile initialization so hermetic tests cannot race temporary-storage cleanup | Add physical disk-full/crash-restart reconciliation and release-level recovery rehearsal |
| QA-8 | Local test confidence | PARTIAL | Full audit at `de7943a` passes 1,300 host tests and 77 webview tests across 18 files; exact host coverage is 92.09% lines/84.77% branches/88.50% functions and webview coverage is 96.13% lines/90.64% branches/94.38% functions; architecture is checked for 437 production TypeScript files, webview boundary floors remain enforced, proxy-settings application/restoration/backup, VS Code window-configuration, active-conversation presentation, profile proxy edit, `EfficiencyAnalyzer`, QuotaClient, certificate recovery, protobuf analysis CLI, protobuf descriptor extraction, protobuf JSONL route/evaluation, webview diagnostic-bridge, Accounts panel dispatcher, Accounts panel lifecycle, proxy runtime/certificate/output handlers, `ProxyOutputPresenter`, proxy runtime composition, agent tracking repository-pool concurrency/close/error paths, typed ProxyManager construction/branch behavior, session-token capture/accounting/report rendering, webview message-state family boundaries, ProfileCard identity/actions behavior, efficiency database lifecycle/recovery, proxy traffic usage coordination, profile import feedback, agent extraction, proxy traffic analysis, efficiency output presentation, and profile launch presentation/command behavior have direct contracts, the efficiency persistence read adapters and compressed agent payload path are covered through integration contracts, the webview profile-action hooks are covered through App integration, and the extension initialization workflow has direct contracts; merged Repowise coverage retains 397 files at 92.5% lines and 84.7% branches | Standardize repeated static-analysis coverage ingestion and cover the next process, storage, and release hotspots |
...
| Latest implementation commit | de7943a |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 437 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-28 at commit `de7943a`
with the follow-up filesystem-error hardening at `42ed7a8`, RunSSE serialization
at `88d35fe`, response-finalization context extraction at `b241bc5`, profile
import/export command decomposition at `f5e7958`, and CRUD command failure-path
coverage at `886ea86`, followed by accounts-panel launch workflow extraction,
remaining Accounts panel presentation-workflow extraction, refresher coverage
hardening, active-conversation totals isolation, profile proxy lifecycle
decision isolation, certificate material generation extraction with single-flight
initialization, strict propagation of non-missing certificate material read
failures, status-bar display-policy extraction, binary decoder recovery-path
isolation and coverage, traffic-summary/domain and UI/presentation boundary
extraction, proxy-settings application/restoration/backup responsibility
separation, direct VS Code proxy-window adapter coverage, and removal of the
proxy-settings compatibility facade, followed by the split of efficiency
database aggregate, event, and quota reads into separate injected adapters,
and consolidation of gzip/Connect payload candidate decoding for agent streams,
followed by webview profile-action bridge/form separation, shared proxy
relationship enrichment, extension window initialization coverage,
active-conversation status presentation extraction with shared token/cost
formatting, and profile proxy edit persistence/lifecycle separation with
explicit settings restoration and modeled runtime actions, followed by direct
`EfficiencyAnalyzer` facade coverage for queue and error contracts, and the
typed ProxyManager construction-options and environment/URL branch contracts.
The subsequent QuotaClient contract slice covered empty and unreadable HTTP
error bodies, the IDE-to-web fallback, IDE preference when the optional web
source fails, successful IDE/web merging, and the normalized both-sources
failure. The complete audit remained green after these additions.
The subsequent CertificateManager recovery slice covered rebuilding a missing
public key from an existing certificate and regenerating material when an
existing certificate cannot be parsed. The complete audit remained green after
these additions.
The subsequent protobuf-analysis CLI slice added the missing `protobufjs`
runtime imports and smoke-tested both executable entrypoints against empty log
directories. The complete audit remained green after these additions.
The subsequent webview adapter slice covered diagnostic message forwarding and
early-boot logging when the inline VS Code API is unavailable. The complete
audit remained green after these additions.
The subsequent Accounts panel dispatcher slice added a typed contract for all
specialized webview action routes and the unknown-message no-op. The complete
audit remained green after these additions.
The subsequent Accounts panel provider lifecycle slice added explicit contracts
for panel disposal, reveal behavior without an active panel, runtime-ready
refreshes, visible/hidden panel transitions, workspace-change refreshes, empty
workspace auto-opening, and missing-bundle diagnostics. The complete audit
remained green after these additions.
The subsequent session-token slice extracted state accounting and report
calculation into `scripts/lib/session-token-summary.mjs`, extracted text
rendering into `scripts/lib/session-token-report.mjs`, and made the three
contract tests a mandatory audit stage. The complete audit remained green
after these additions.
The subsequent session-capture slice extracted JSONL parsing, RPC/event
classification, protobuf/body decoding, stream processing, and file-level
analysis into `scripts/lib/session-token-capture-analysis.mjs`. Its direct
contracts cover malformed/non-directional entries, billing, BidiAppend,
RunPoll, and RunSSE paths. The complete audit remained green after these
additions.
The subsequent webview message-state slice split the reducer into typed host,
proxy, storage, and pricing message-family boundaries while preserving the
public reducer contract. The webview suite passed 74 tests across 18 files,
and the complete audit remained green after the change.
The subsequent ProfileCard slice separated identity and metadata rendering
from launch, menu, confirmation, and outside-click lifecycle behavior. The
webview suite passed 77 tests across 18 files, and the complete audit remained
green after the change.
The subsequent efficiency persistence lifecycle slice moved schema
initialization, migration recovery, database sizing, and destruction behind a
focused lifecycle boundary, added pre-probe WAL/SHM recovery snapshots, and
covered fresh, existing, corrupted, sidecar, size, and destroy contracts. The
focused lifecycle suite passed 4/4 tests, and the complete audit remained
green after the change.
The subsequent proxy traffic usage slice split local ingestion, shared-runtime
notification, profile resolution, and logging behind focused coordinator
methods, replaced concrete service/global logger dependencies with injected
capabilities, and added four direct branch contracts. The focused coordinator
suite passed 6/6 tests, and the complete audit remained green after the change.
The subsequent profile import slice moved localized result aggregation,
refresh eligibility, and success/error message selection into a pure
presentation policy. Its direct policy suite passed 3/3 tests, the existing
profile-handler suite remained green, and the complete audit remained green
after the change.
The subsequent protobuf descriptor slice separated message-factory,
message-reference, enum, and service extraction responsibilities, added
malformed and unsupported-fragment contracts, and kept package filtering and
descriptor output compatibility unchanged. The focused extraction suite
passed 7/7 tests, and the complete audit remained green after the change.
The subsequent protobuf JSONL verification slice extracted pure entry-body
evaluation and route/content classification, grouped runtime dependencies into
explicit contexts, and added direct success, truncation, malformed-route, and
empty-directory contracts. The focused verifier suite passed 8/8 tests, and
the complete audit remained green after the change.
The subsequent agent extraction slice separated Connect framing and decoding,
recursive input/output/misc text-bucket extraction, and stream/prefetch token
metrics into three pure modules while preserving the historical import path
through a compatibility barrel. The focused agent suite passed 3/3 tests, and
the complete audit remained green after the change.
The subsequent efficiency presentation slice moved output formatting,
inefficiency-threshold decisions, recommendation text, and high-severity
notification requests into a VS Code-free application policy. The VS Code
adapter now owns only localization, configuration, output-channel operations,
and notification execution. Its focused policy and adapter suites pass 11/11,
and the complete audit remained green after the change.
The subsequent profile launch slice moved quick-pick item construction and
date-label handling into a VS Code-free presentation policy. Direct command
contracts cover empty-profile creation, cancellation, selection cancellation,
executable validation, successful and failed launches, and unexpected errors;
the policy and command suites pass 10/10, and the complete audit remained
green after the change.
The complete audit included:

- extension-host tests: passed across the compiled host test suite;
- 1,300 host tests and exact coverage of 92.09% lines, 84.77% branches, and
  88.50% functions;
- architecture rules and fixtures: passed for 437 production TypeScript files;
- localization: 26 locales with 428 keys each;
- webview tests: 18 files and 77 tests passed, with 96.13% lines, 90.64%
  branches, and 94.38% functions under the V8 coverage gate;
- current VSIX content verification: passed;
- agent extraction contracts: 3/3 passed, including raw/gzip/incomplete
  framing, text classification/hex decoding, identifier filtering, metric
  normalization, prefetch deduplication, and legacy token estimates.

The session-token accounting contract suite passed 5/5 tests, including
spending normalization, peak grouping, duplicate-event handling, report
construction, and the empty-capture CLI smoke path.

The complete host suite passed as part of the audit with zero failures,
cancellations, or skips. Running the compiled suite with an unrelated global
Node runtime is not valid evidence because the native SQLite binding is
ABI-specific; such invocations are intentionally excluded from the quality
record.
...
The latest graph was generated with Graphify `0.9.48` from the current code
tree at `de7943a`, with documentation, generated output, and dependency
directories excluded, using no clustering. It scanned 917 code files and
contains 5,498 nodes and 15,550 raw edges; the normalized post-build
diagnostic graph contains 13,278 directed edges.
The highest relevant
hotspots include:

- `t()`: degree 127;
- ProxyTrafficSummary: degree 87;
- IProfileReader: degree 72;
- scripts: degree 71;
- IProfileDetector: degree 69;
- AgentSessionInfo: degree 45;
- ProxyManager: degree 44 in the god-node ranking; it remains an intentional
  public coordination boundary after event/diagnostics extraction and now
  accepts one typed construction-options object;
- IAgentTrackingRepository: degree 41;
- IProfileManager: degree 41;
- VSCodeAPI: degree 41;
...
directed multigraph diagnostic reports 2,107 dangling endpoint edges, one
self-loop, no missing endpoints, and 164 same-endpoint relation groups; it
retains 13,443 valid candidate edges and collapses them to 13,278 normalized
...
- Safe-only dead-code analysis: no findings. The current index reports zero
  unreachable files and zero unused exports; the unrestricted report
  still contains low-confidence configuration, public-contract, and package
  signals, so no deletion was performed. This is evidence that no safe
...
- Historical security scan: 0 findings across 214 commits, 4,539 blobs, and
  2,578 files.
- Current refactoring targets include `packages/types/src/index.ts`,
  `src/commands/profileCommands.ts`, `scripts/verify-proto-jsonl.mjs`,
  `src/domain/ports/IProxyManager.ts`, `src/application/types/agentPersistence.ts`,
  `src/application/types/proxyInsights.ts`, `src/application/types/proxyTraffic.ts`,
  `src/profiles/types.ts`, `src/proxy/types.ts`,
  `src/services/proxyCertificateService.ts`, `src/proxy/proxyInsightExtractor.ts`,
  `src/proxy/proxyServer.ts`, `scripts/extract-cursor-protos.mjs`,
  `scripts/analyze-proxy-traffic.mjs`, `src/ui/accountsPanelDataRefresher.ts`,
  `src/services/agentTrackingService.ts`, and `src/proxy/requestLogger.ts`.
  These are prioritization candidates only:
  each must first receive a capability/dependency review and characterization
  coverage, and no split is justified by a score alone. The former
  `proxySettingsService.ts` target is no longer current because the
  compatibility facade was removed from production wiring.

The latest Repowise index covers the code tree after `de7943a` and reports
5,720 nodes with 15,489 edges; the host and webview lcov reports were
explicitly merged with `repowise coverage add
coverage/lcov.info webview/coverage/lcov.info`. The latest ingestion accepted
398 file entries (355 exact and 43 resolved mappings); the retained coverage
index reports 397 files, 92.5% lines, and 84.7% branches. The latest health
snapshot reports average health at 9.01/10, hotspot health at 6.91/10, and
worst-performer health at 4.15/10 for `packages/types/src/index.ts`; the
performance scan reports 98 findings at 12.53 per 10K covered LOC. These findings are
prioritization signals, not release gates.

The current efficiency presentation policy scores 9.70/10 with NLOC 97,
maximum CCN 5, maximum nesting 2, and 100.00% line/branch coverage; Repowise
retains only low-severity DRY and parameter-grouping heuristics. The VS Code
adapter has 100.00% line/branch coverage over 120 coverable lines, while its
remaining Repowise coverage-gradient marker is treated as a heuristic because
the authoritative LCOV record is complete. The profile launch policy scores
10.00/10 with 100.00% line/branch/function coverage; the command adapter has
100.00% line coverage, 81.25% branch coverage, and 100.00% function coverage.
No dependency-direction violation or safe-only dead-code finding was introduced.

The Accounts panel proxy action facade is now composed from dedicated runtime,
certificate, and output adapters. Repowise's current health report is 9.00/10
average and 6.91/10 hotspot health. It flags low-cohesion and cross-module
duplication heuristics in the runtime/certificate adapters and historical
change entropy in the compatibility facade; these are prioritization signals,
not dependency-direction violations. The focused host suite covers successful,
missing-profile, failure, certificate-recovery, and VS Code output-command
paths. The output adapter has no findings, and safe-only dead-code analysis is
empty.

The proxy server composition root now has direct coverage for both the
production/no-tracking configuration and the development/tracking/
multi-profile-mapping configuration. Both contracts compose the real runtime
and shut it down without opening listeners, so they validate dependency wiring
and optional-boundary selection without requiring network access. The focused
proxy-server suite passes 9/9, and the full audit remains green.

The agent-tracking repository pool now serializes concurrent initialization
per profile with a single-flight promise boundary. Profile and global close
operations wait for pending initialization before clearing the pool, and
failed initialization removes its pending state so a later attempt is not
poisoned. The focused SQLite integration contract passes 4/4; LCOV reports
96.93% lines, 93.33% branches, and 100.00% functions for the pool itself.

The agent extraction boundaries are now measured independently. Repowise
reports `scripts/lib/agent-stream-frames.mjs` at 9.20/10 (NLOC 113, maximum
CCN 5, maximum nesting 3), `scripts/lib/agent-text-buckets.mjs` at 9.85/10
(NLOC 140, maximum CCN 7, maximum nesting 2), and
`scripts/lib/agent-token-metrics.mjs` at 9.85/10 (NLOC 113, maximum CCN 7,
maximum nesting 3). The compatibility barrel
`scripts/lib/agent-text-extract.mjs` scores 10.00/10 with NLOC 15 and no
findings. The remaining low/medium DRY markers compare offline analysis code
with either `scripts/calibrate-proxy-tokens.mjs` or the tokenizer-specific
`scripts/lib/ai-token-estimate.mjs`; those modules intentionally retain
different policies or process/runtime boundaries and were not merged through a
generic abstraction. The three decoder catches intentionally recover from
unsupported gzip/framing/ protobuf candidates and are covered by the direct
contracts. The safe-only dead-code report remains empty.

The proxy traffic analysis now has explicit decoder, insight-aggregation, and
file-orchestration boundaries. Repowise reports
`scripts/lib/proxy-entry-decoder.mjs` at 10.00/10 (NLOC 43, maximum CCN 4,
maximum nesting 2, no findings),
`scripts/lib/proxy-entry-insights.mjs` at 10.00/10 (NLOC 51, maximum CCN 4,
maximum nesting 1, no findings), and
`scripts/lib/proxy-traffic-analysis.mjs` at 10.00/10 (NLOC 134, maximum CCN
6, maximum nesting 2). The remaining low-severity synchronous-I/O marker is
intentional for the offline CLI boundary; the focused analysis suite covers
JSON/protobuf/gzip decoding, malformed entries, RPC aggregation, nested token
insights, and the empty-directory executable path.

The protobuf descriptor parser now has separate pure helpers for runtime
factory descriptors, static/quoted references, enums, and services. Repowise
scores `scripts/lib/cursor-proto-descriptors.mjs` at 8.20/10 (NLOC 287,
maximum CCN 13, maximum nesting 3, duplication heuristic 12.43%) and no
longer ranks its message extraction as a refactoring target. The remaining
medium complexity markers are localized to `fieldToProto` and the balanced
delimiter scanner, whose branching represents format parsing and malformed
input recovery; safe-only dead-code analysis remains empty.

The protobuf JSONL verifier now has pure route classification and pure body
evaluation boundaries, with report mutation confined to the orchestration
layer. Repowise reports the module at 7.19/10 after the latest four-commit
change-entropy signal (the structural score before that history marker was
8.85/10); current structural findings are the CCN 11 route orchestrator,
blocking filesystem reads required by the CLI, and intentionally swallowed
malformed-line/body decode errors. Direct tests cover all route classes,
truncated bodies, successful decoding, gzip handling, malformed input, and the
empty-directory executable path.

The proxy traffic usage coordinator now has an explicit application boundary:
the coordinator depends on an ingestion capability and a logger contract
instead of the concrete tracking service and global logger. Repowise scores
`src/services/proxyTrafficUsageCoordinator.ts` at 7.8/10 (NLOC 140, maximum
CCN 8, maximum nesting 2, with a test file). Its remaining markers are medium
low cohesion and change entropy plus low-severity conditional complexity in
the two focused workflow methods; direct contracts cover local ingestion,
shared-runtime notification, missing-profile handling, context conversation
fallback, and non-agent persisted usage.

The profile import handler now delegates localized count aggregation, refresh
eligibility, and success/error message selection to the pure
`src/ui/profileImportFeedback.ts` policy. Repowise scores the policy at 9.8/10
(NLOC 39, maximum CCN 8, maximum nesting 1, with a test file); its only
remaining marker is a low-severity duplication heuristic. Direct contracts
cover successful imports with skips, partial failures, and no-change success
or failure outcomes.

The session-token pipeline now has clean boundaries for accounting, rendering,
and capture analysis. Repowise reports
`scripts/lib/session-token-summary.mjs` at 9.15/10 (NLOC 142, maximum CCN 11,
maximum nesting 2, with a test file),
`scripts/lib/session-token-report.mjs` at 8.78/10 (NLOC 86, maximum CCN 13,
maximum nesting 2, with a test file), and
`scripts/lib/session-token-capture-analysis.mjs` at 9.00/10 (NLOC 167,
maximum CCN 9, maximum nesting 2, with a test file). The executable
`scripts/summarize-session-tokens.mjs` is now 9.65/10 (NLOC 47, maximum CCN 6,
maximum nesting 1); its remaining findings are an I/O-in-loop review and
historical co-change signals. The webview message-state reducer has now been
split into family-specific modules. Repowise reports the public reducer at
9.34/10 (NLOC 137, maximum CCN 5, maximum nesting 1, 98.41% lines, and
95.24% branches), `appMessageStateHost.ts` at 9.65/10 (NLOC 105, maximum CCN
2, 100% lines and branches), `appMessageStateProxy.ts` at 9.65/10 (NLOC 83,
maximum CCN 7, maximum nesting 3, 100% lines and branches),
`appMessageStateStorage.ts` at 9.65/10 (NLOC 33, maximum CCN 5, 100% lines
and branches), and `appMessageStatePricing.ts` at 9.65/10 (NLOC 26, maximum
CCN 2, 100% lines and branches). The remaining duplication biomarkers are
low-to-medium heuristic signals caused by similar immutable state updates;
the modules remain cohesive and are covered by direct webview contracts.

The ProfileCard UI slice now has explicit identity and actions boundaries.
Repowise reports `webview/src/components/ProfileCard.tsx` at 6.15/10 (NLOC
127, maximum CCN 3, maximum nesting 1, 100% lines and 96% branches),
`ProfileCardActions.tsx` at 9.65/10 (NLOC 109, maximum CCN 4, 100% lines and
branches), and `ProfileCardIdentity.tsx` at 10.00/10 (NLOC 103, maximum CCN
1, 100% lines and branches). The remaining score of the public card is
history-based change entropy/churn rather than uncovered behavior; the
component is now a compositor of focused presentation boundaries.

Repowise now reports `src/services/proxyManager.ts` at 6.41/10 (NLOC 315,
maximum CCN 3, maximum nesting 1, 97.48% lines and 100.00% branches over 357
coverable lines). Its remaining high-severity markers are co-change scatter and
low cohesion in the public facade. The class is intentionally retained as an
adapter implementing the segregated lifecycle, status, certificate, output,
and routing ports; no unrelated domain behavior was moved into it. The typed
options object reduces composition ambiguity, while any future extraction must
be justified by a concrete capability boundary and characterization contracts,
not by the heuristic alone.

The former `src/services/proxySettingsService.ts` hotspot was removed rather
than carried forward: production now wires the application, restoration, and
backup-reader services directly, and no safe-only dead-code finding remains.
The efficiency persistence read side is now split into three injected
functional adapters: `efficiencyAggregateReader.ts` owns profile, repository,
and branch projections; `efficiencyEventReader.ts` owns filtered event reads;
and `efficiencyQuotaReader.ts` owns quota-range and trend projections. The
public `EfficiencyDatabase` facade retains writes, migration/recovery, and
backward-compatible read methods while composing these narrow boundaries.
Repowise reports the database facade at 5.8/10 (NLOC 183, max CCN 3, max
nesting 1); the aggregate reader at 9.52/10
(NLOC 185, max CCN 5, max nesting 1, 91.63% lines and 75.00% branches); the
event reader at 9.85/10 (NLOC 168, max CCN 8, max nesting 2, 100.00% lines
and 85.71% branches); and the quota reader at 10.00/10 (NLOC 79, max CCN 1,
max nesting 0, 100.00% lines and 75.00% branches). The remaining database
findings are coverage/history/error-handling signals; the readers have no
cohesion or complexity finding. Their behavior is covered through the
`EfficiencyDatabase` integration contracts, including repository, branch,
quota phase, range, and trend projections.
The agent stream decoder now shares one pure gzip/Connect candidate walker
between direct `AgentServerMessage` decoding and `HealthResponse.payload`
decoding. This removes duplicated framing behavior without coupling the
runtime decoder to the separate protobuf-analysis scripts. Repowise reports
`src/proxy/agentStreamDecode.ts` at 7.26/10 (NLOC 182, max CCN 7, max nesting
4, 92.20% lines and 77.08% branches); its remaining findings are the helper's
medium nesting, cross-tooling duplication heuristic, two intentionally
defensive decode catches, and recent churn. The decoder is covered by its
existing stream contracts plus a gzip-compressed Connect regression test.
The webview profile actions are now composed from two focused hooks: the
bridge hook owns host-only profile commands, while the form hook owns editing,
creation, and persisted UI-state transitions. The facade contains only typed
composition. Repowise reports the facade at 10.00/10 (NLOC 46, maximum CCN 1,
maximum nesting 0), the bridge hook at 9.65/10 (NLOC 64, maximum CCN 1,
maximum nesting 0, 100% line/branch coverage), and the form hook at 9.62/10
(NLOC 107, maximum CCN 5, maximum nesting 1, 100% line/branch coverage).
Repowise's standalone-test-file flag is false for the two extracted hooks
because their behavior is covered through `App.test.tsx`; the V8 report still
records 100% direct line and branch coverage for both files. The bridge hook's
self-clone marker is a low-value heuristic caused by repeated host-message
shapes, so no generic dynamic sender abstraction was introduced.
The proxy insight enricher now applies conversation and conversation-group
relationships through one shared pure policy for Bidi and agent-stream paths.
Repowise reports the enricher at 8.38/10 (NLOC 149, maximum CCN 7, maximum
nesting 2, 77.84% lines and 60.71% branches); the remaining markers are a
coverage gradient and prior-defect history, with no new duplication or
dependency-direction finding.
The extension activation workflow now delegates current-window detection,
existing proxy attachment, and accounts-panel auto-open decisions to one
focused composition function. This keeps the activation orchestrator at the
high-level lifecycle outline while preserving the stale-activation cleanup
guard at the caller. Repowise reports `src/composition/extensionInitialization.ts`
at 9.53/10 (NLOC 97, maximum CCN 8, maximum nesting 2, 88.14% lines and
60.00% branches); the remaining coverage gradient is now characterized by
direct contracts for unassigned windows, active proxy windows, and stale
activation after shared-proxy startup.
The active-conversation status bar now delegates all user-visible formatting,
tooltip composition, warning-state decisions, and token/cost formatting to
VS Code-free presentation policies. Repowise reports
`src/ui/presentation/activeConversationStatusBarPresentation.ts` at 10.00/10
(NLOC 232, maximum CCN 5, maximum nesting 1, 100.00% lines, 98.15% branches,
and 4.72% duplication) with no findings; the shared
`tokenCostFormatting.ts` helper has 100.00% direct line and branch coverage.
The VS Code status-bar adapter retains only lifecycle, loading, visibility, and
ThemeColor side effects. The focused status-bar suite passes 12/12 contracts,
including empty, active, context, billed/live, sub-cent, and formatting-boundary
cases.
The `EfficiencyAnalyzer` facade now has direct contracts for workflow
delegation, duplicate queue admission, `Error` failures, and arbitrary runtime
rejection values. Its current LCOV record is 100.00% for lines and branches.
Repowise still reports the historical hotspot score at 5.26/10 (NLOC 52, max
CCN 2, max nesting 1, 26.92% duplication), driven by churn and co-change
signals rather than an uncovered behavior path. Repowise's `has_test_file`
flag remains false because the repository keeps tests under `src/test`; this
does not match the checked-in
`src/test/modelEfficiency/efficiencyAnalyzer.test.ts` file, so LCOV and the
focused suite are the authoritative direct-test evidence.
`QuotaClient` remains an API-layer adapter rather than an application policy:
it obtains valid tokens through the domain port, calls the validated IDE and
web usage sources, and applies the existing source-precedence mapper. Its
Repowise score remains 5.39/10 (NLOC 120, maximum CCN 7, maximum nesting 3,
26.67% duplication), but its current LCOV record is 100.00% for lines, branches,
and functions. The remaining Repowise markers are medium historical
co-change/change-entropy signals; no dependency-direction or uncovered
behavior finding remains. The focused quota fetch suite passes 10/10.
`CertificateManager` now has direct contracts for missing-public-key recovery
and malformed-certificate regeneration, in addition to generation reuse,
single-flight behavior, non-missing read-error propagation, and the MITM
directory layout. Repowise reports the manager at 5.63/10 (NLOC 97, maximum
CCN 8, maximum nesting 3), with 98.39% line and 95.83% branch coverage. Its
remaining markers are historical change entropy and churn risk; no current
dependency-direction, duplication, or untested recovery-path finding remains.
The focused CertificateManager suite passes 8/8, and the two residual uncovered
lines correspond to the defensive propagation branch for an unexpected public
key access error.
Both protobuf-analysis CLI entrypoints now import `protobufjs` explicitly and
have executable smoke contracts against empty log directories. Repowise reports
`scripts/analyze-proxy-traffic.mjs` at 5.05/10 (NLOC 53, maximum CCN 9,
maximum nesting 2, 35.00% duplication) and
`scripts/verify-proto-jsonl.mjs` at 5.90/10 (NLOC 36, maximum CCN 3, maximum
nesting 1, 27.91% duplication). Their remaining markers are critical
historical change-entropy/churn signals; the `has_test_file` heuristic is false
because the smoke contracts live in the shared analysis/verifier test modules.
The webview `VSCodeAPI` remains a deliberately thin transport adapter with
typed command wrappers, message buffering, state-version validation, and
early-boot diagnostic isolation. Its direct LCOV is now 100.00% for lines,
branches, and functions after adding contracts for diagnostic forwarding and
the unavailable inline API path. Repowise still reports historical
change-entropy and co-change signals for this adapter; no dependency-direction
or behavioral design defect was found, so no artificial wrapper extraction was
introduced.
`AccountsPanelProvider` remains a UI composition and lifecycle adapter rather
than a domain service: its constructor assembles injected coordinators and
typed handlers, while webview HTML, message routing, read-model refreshes, and
proxy/workspace state are delegated to dedicated boundaries. Repowise reports
the provider at 5.73/10 (NLOC 356, maximum CCN 4, maximum nesting 2, and
31.62% duplication) with 95.66% direct line coverage and 95.45% direct branch
coverage. The lifecycle contracts now cover disposal, no-panel reveal, the
runtime-ready refresh path, visible/hidden transitions, workspace changes,
empty-workspace auto-opening, and missing-bundle diagnostics. The remaining
score is driven by historical change entropy/churn and the composition surface;
no new dependency-direction violation was found, so no artificial extraction
was introduced.
Profile edits now pass through `ProfileProxyEditUseCase`, while
`ProfileProxyEditLifecycleService` owns the explicit `stop`, `ensure`,
`restart`, or `none` runtime decision. This removes direct proxy-settings
restoration from the UI handler and prevents a production double-restore path.
Repowise reports the use case at 10.00/10 (NLOC 28, maximum CCN 1, maximum
nesting 0, 100.00% lines and 100.00% branches) and the lifecycle service at
9.85/10 (NLOC 80, maximum CCN 8, maximum nesting 2, 100.00% lines and
100.00% branches). The lifecycle file retains one low-severity DRY heuristic
against a semantically different active-conversation loader; no generic
abstraction was introduced. The focused profile-edit contracts pass 11/11,
including disabled, current-window, inactive-window, disabled-result,
JSONL-change, unchanged, and stopped-runtime cases.
The extracted application boundaries report
`proxySettingsApplicationService.ts` at 9.31/10 (NLOC 72, max CCN 3, max
nesting 3, 95.18% lines and 87.50% branches),
`proxySettingsRestorationService.ts` at 9.74/10 (NLOC 53, max CCN 4, max
nesting 3, 93.44% lines and 77.78% branches), and
`proxySettingsBackupReader.ts` at 9.39/10 (NLOC 39, max CCN 2, max nesting 2,
84.78% lines and 80.00% branches); their remaining findings are coverage
gradients only, except for the application service's low-severity cohesion and
duplication signals recorded by Repowise.
...
`src/ui/presentation/proxyTrafficLineFormatter.ts` (9.81/10, NLOC 112, max CCN 8, max nesting
2, 95.28% lines and 70.00% branches; remaining finding is a low coverage-gradient
signal), `src/proxy/proxyTrafficSummary.ts` (9.85/10, NLOC 99, max CCN 7, max
nesting 3, 96.26% lines and 86.05% branches; remaining finding is a low
coverage-gradient signal), `src/services/multiProfileQuotaService.ts` (6.30/10,
NLOC 160, max CCN 3,
...
`src/proxy/certificateManager.ts` (5.63/10, NLOC 97, max CCN 8, max nesting 3,
98.39% lines and 95.83% branches; the remaining signals are historical
change-entropy and churn-risk markers), `src/proxy/certificateMaterialGenerator.ts` (10.00/10, NLOC 35,
max CCN 1, max nesting 0, 100% lines and 100% branches),
`src/ui/statusBarManager.ts` (5.79/10, NLOC 117, max CCN 4, max nesting 2,
91.11% lines and 90.91% branches; remaining signals are adapter duplication
and churn), `src/ui/statusBarDisplayPolicy.ts` (9.15/10, NLOC 153, max CCN 10,
max nesting 1, 100% lines and 88.89% branches),
`src/proxy/proxyBinaryDecoder.ts` (9.88/10, NLOC 146, max CCN 7, max nesting
3, 96.91% lines and 81.25% branches; the remaining signal is a small
coverage gradient),
`src/services/proxyManager.ts` (4.77/10, NLOC 289, max CCN 3, max nesting 1,
95.45% lines and 93.62% branches),
`src/application/services/proxyProfileLifecycleUseCase.ts` (9.65/10, NLOC 53,
max CCN 3, max nesting 1, 100% lines and 100% branches),
...
`scripts/analyze-proxy-traffic.mjs` (5.05/10, NLOC 53, max CCN 9, max
nesting 2), `scripts/lib/proxy-traffic-analysis.mjs` (7.20/10, NLOC 175,
max CCN 13, max nesting 4),
`scripts/verify-proto-jsonl.mjs` (5.90/10, NLOC 36, max CCN 3, max nesting 1),
...
`src/proxy/mitmProxyServer.ts` (6.19/10, NLOC 238, max CCN 4, max nesting 3,
92.31% lines and 75.00% branches),
`src/proxy/proxyLogTailer.ts` (6.54/10, NLOC 200, max CCN 6, max nesting 3,
87.18% lines and 78.00% branches), while its file discovery and entry
processing responsibilities now score 9.78/10 and 9.37/10 respectively;
`src/ui/accountsPanelHandlers.ts` (5.38/10, NLOC 221, max CCN 2, max nesting
1, 100.00% lines and 100.00% branches), while the extracted
`src/ui/accountsPanelLaunchHandlers.ts` scores 9.85/10 with NLOC 117, maximum
CCN 6, maximum nesting 3, 100% lines, 88.00% branches, and 11.86%
duplication. The efficiency and suggested-profile adapters both score 9.65/10;
the profile adapter scores 7.79/10 with NLOC 166, maximum CCN 11, maximum
nesting 2, 94.68% lines, and 75.00% branches;
`src/ui/accountsPanelDataRefresher.ts` now scores 5.55/10 with NLOC 93,
maximum CCN 3, maximum nesting 2, and 100% direct line and branch coverage;
the remaining `AccountsPanelHandlers` coordinator now has 100% direct line and
branch coverage for all 22 specialized webview routes and the unknown-message
no-op. Its score remains 5.38/10 (NLOC 221, maximum CCN 2, maximum nesting 1,
58.59% duplication) because Repowise attributes the residual risk to historical
change entropy and churn rather than current complexity or missing behavior.
...
The latest accounts-panel and logger review is also explicit: `src/ui/accountsPanel.ts`
scores 5.41/10 at NLOC 356 with 87.71% lines and 90.62% branches; the
provider now receives one named dependency object, so its constructor no longer
has a primitive-obsession finding. `src/ui/accountsPanelDataRefresher.ts`
scores 4.50/10 at NLOC 93 with 73.87% lines and 90.91% branches; its remaining
duplication, cohesion, churn, and entropy markers are recorded as follow-up
signals because it is a compatibility coordinator delegating to the dedicated
initial-data, proxy-state, workspace-state, and background-refresh boundaries.
The extracted initial-data and workspace-state coordinators retain direct
contracts and 100% line/branch coverage. `src/github/profileGitHubEnrichmentService.ts`
scores 8.61/10 at NLOC 171 with 98.96% lines and 93.33% branches after direct
coverage for public/private/rate-limited/not-found metadata and per-workspace
failure isolation. `src/proxy/requestLogger.ts` is now a small compatibility
facade around the dedicated `src/proxy/proxyLogStorage.ts` filesystem adapter.
The logger retains body-capture and metadata compatibility, while storage owns
serialized JSONL writes, rotation, deterministic ordering, and bounded body
pruning. The direct logger/storage suites cover nine contracts, including the
previously untested rotation queue deadlock, oldest-file retention, and
sidecar pruning. The storage adapter reports 87.30% lines, 79.54% branches,
and 91.66% functions in the current host coverage report.

Profile import/export command registration is now split into a small
composition function plus separate VS Code adapters for export and import.
`registerProfileImportExportCommands.ts` is a 10.00/10 composition boundary
with NLOC 11, maximum CCN 1, and 100% direct line/branch coverage.
`registerProfileExportCommand.ts` scores 8.59/10 at NLOC 91 with maximum CCN
7, maximum nesting 3, 82.83% line coverage, and 61.54% branch coverage;
`registerProfileImportCommand.ts` scores 8.34/10 at NLOC 97 with maximum CCN
8, maximum nesting 3, 67.31% line coverage, and 50.00% branch coverage.
Repowise flags the command-local dialog workflows for low-severity size or
coverage gradients and medium heuristic duplication; these are adapter-level
follow-ups to be addressed with cancellation, validation, warning, and
failure-path contracts rather than by introducing domain dependencies.

The response adapter is now explicitly serialized at the stream boundary.
`src/proxy/mitmProxyResponseHandler.ts` queues each RunSSE chunk in arrival
order and makes response finalization await that queue before calling
`StreamingAgentDecoder.finalize()` and removing decoder state. This prevents a
fast response end or delayed protobuf initialization from dropping live token
updates. The response lifecycle and RunSSE finalization data are represented by
typed contexts rather than long positional parameter lists; the remaining
composition callback is intentionally kept at the transport boundary.
Repowise scores the adapter at 6.96/10 with NLOC 290, max CCN 10, max nesting 2,
96.03% line coverage, 73.33% branch coverage, and 14.92% duplication. Its
remaining findings are the callback's composition complexity, a low-severity
duplication/coverage signal, and historical churn/defect markers. The direct
response-handler suite covers the normal lifecycle and the delayed-registry
initialization race; the complete audit passes at `b241bc5`.

The agent insight extractor is now a compatibility barrel over focused
`agentExtractionSupport`, `agentUsageExtraction`, and
`agentRequestExtraction` policies. Usage validation reaches 9.81/10 in
Repowise at NLOC 207 with max CCN 6, 95.19% lines, and 82.19% branches;
relationship extraction reaches 9.12/10 at NLOC 196 with max CCN 7, 96.74%
lines, and 75.56% branches. The remaining relationship duplication marker is
intentionally retained because the explicit typed adapters are clearer and
safer than a dynamic field-spec abstraction; no behavior or
dependency-direction suppression was added.
...
an intentional composition root; its certificate/logger/proto/handler/listener
stages now have private composition helpers and transactional cleanup, while
the lifecycle seams retain direct characterization and failure-path coverage.
...
Repowise sync metadata points to commit `7bca506`, with no embedding provider or
...
## QA-5 / QA-8.79 profile workspace application boundary — 2026-08-27

The profile workspace query service is now an application service in commits
`1f3071c` and `0f96c1b`. It depends on `IProfileReader` and the new domain
`IWorkspaceScanner` port; the filesystem-backed `WorkspaceScanner` remains an
adapter in the `profiles` layer and is assembled only at the composition root.
The previous service-layer import of the concrete scanner has been removed.

Evidence for this checkpoint:

- the direct application contract passes 3/3, covering all-profile
  composition, unknown-profile short-circuiting, existing-profile delegation,
  and most-recent workspace selection;
- the complete audit passes on commit `0f96c1b` under Node `24.19.0` and pnpm
  `10.34.0`, with 1,112 host tests, 69 webview tests across 18 files, and
  architecture checks covering 389 production TypeScript files;
- Repowise reports `src/application/services/profileWorkspaceService.ts` at
  10.00/10 with NLOC 34, maximum CCN 2, maximum nesting 1, 100% line and
  branch coverage, and no current findings. Removing the unused barrel export
  returns the general unused-export report to zero, while the safe-only
  dead-code report remains empty;
- the merged Repowise coverage ingestion accepts 345 entries (312 exact and
  33 resolved) and retains 344 files at 87.58% lines and 80.38% branches;
- Graphify `0.9.48` code-only extraction reports 822 code files, 4,953 nodes,
  13,888 raw edges, and 11,839 directed post-build edges. Its diagnostics
  report 1,906 dangling endpoint edges, one self-loop, no missing endpoints,
  142 same-endpoint relation groups, and 11,982 valid candidate edges.

This is a dependency-direction and testability improvement with no intended
runtime behavior change. QA-5 and QA-8 remain PARTIAL because broader
composition, release, and reliability criteria are still open.

## QA-5 / QA-8.80 accounts-panel proxy-state boundary — 2026-08-27

The accounts-panel proxy read path is now isolated behind
`AccountsPanelProxyStateCoordinator` in commit `f11f8e6`. The coordinator owns
proxy-state read composition, certificate-status derivation, temporary-profile
classification, and guarded webview refresh publication. The panel refresher
now consumes this boundary instead of knowing the concrete proxy manager,
settings service, proxy-status policy, or profile-proxy predicate.

The concrete settings service implements the narrow domain-facing
`IProxySettingsBackupReader` port. The port is imported directly by its
consumers rather than re-exported from a broad barrel because Repowise showed
that a barrel export would be an unused public symbol. The returned
`AccountsPanelProxyState` shape is private to the coordinator because callers
only consume the read result structurally.

Evidence for this checkpoint:

- five direct coordinator contracts cover disabled profiles, missing proxy
  capability, live certificate checks, cached certificate checks, temporary
  profile detection, webview refresh publication, and inactive-webview no-op
  behavior;
- the existing refresher contracts continue to pass, for 8/8 combined focused
  proxy-state/refresher tests;
- the complete audit passes on commit `63411e0` under Node `24.19.0` and pnpm
  `10.34.0`, with 1,112 host tests, 69 webview tests across 18 files, zero
  failures/cancellations/skips, 392 architecture-checked production files,
  and successful current VSIX verification;
- host c8 coverage remains 86.85% lines, 81.02% branches, and 84.56%
  functions; webview V8 coverage remains 93.96% lines, 84.85% branches, and
  92.54% functions;
- Repowise `0.45.0` reports the coordinator at 9.46/10 with NLOC 104,
  maximum CCN 7, maximum nesting 2, 95.24% line coverage, and 88.89% branch
  coverage. Its only current signals are low cohesion and a small coverage
  gradient; no safe-only dead-code finding is present;
- the remaining refresher is still the repository's worst health hotspot at
  3.71/10: NLOC 183, maximum CCN 3, maximum nesting 2, 63.29% line coverage,
  90.91% branch coverage, and 26.63% duplication. Its remaining findings are
  low cohesion, a large `refresh` method, duplication against its test, and
  historical churn/change entropy. These are the next UI refactoring targets;
- the merged Repowise coverage ingestion accepts 346 entries (313 exact and
  33 resolved) and retains 345 files at 87.82% lines and 80.67% branches;
- Graphify `0.9.48` code-only extraction reports 827 code files, 4,976 nodes,
  13,955 raw edges, and 11,888 directed post-build edges. Diagnostics report
  1,924 dangling endpoint edges, one self-loop, no missing endpoints, 142
  same-endpoint relation groups, and 12,031 valid candidate edges. The
  increased graph size is expected from the new coordinator, port, and test
  boundaries; the diagnostic counts remain structural signals rather than
  architecture failures.

This checkpoint improves dependency direction, composition isolation, and
testability without changing the accounts-panel proxy behavior by intent. QA-5
and QA-8 remain PARTIAL because the remaining composition hotspot, release
matrix, physical recovery, attribution, and final rehearsal criteria are open.
...
## QA-5 / QA-8.81 accounts-panel composition and request-logger reliability — 2026-08-27

This checkpoint records the follow-up work after the accounts-panel proxy-state
boundary. It is implemented across commits `da4e248`, `bc1995b`, `a8d4099`,
`0764a2a`, `7ed629a`, `87a896f`, `54dd2d5`, and `42ed7a8`.

The accounts panel composition is now explicit at three levels:

- `AccountsPanelInitialDataReader` owns the complete initial read model and
  keeps profile, quota, instance, workspace, efficiency, proxy, locale, and
  message reads together as one application-facing operation;
- `AccountsPanelWorkspaceStateCoordinator` owns open-workspace and running-
  instance projections, while the existing proxy-state and background-refresh
  coordinators retain their respective responsibilities;
- `AccountsPanelProvider` receives one named dependency object at the
  composition boundary, removing the previous 15-position constructor and
  making wiring changes reviewable without positional ambiguity.

The GitHub enrichment service now has direct structural tests for public,
authenticated-private, private, rate-limited, not-found, metadata-error, and
per-workspace failure outcomes. The tests inject structural fakes and do not
perform network access. This protects the application behavior without making
the domain depend on GitHub or a live service.

The request logger received two reliability fixes. Rotation now closes only the
underlying stream from inside the serialized write queue; calling the public
`close()` method from that queue would await itself and deadlock. Filesystem
fallbacks now tolerate only `ENOENT`; permission, busy, storage, and other
errors propagate so storage-limit and logging failures cannot be silently
misclassified as an empty log directory.

Evidence for this checkpoint:

- the focused initial-data suite passes 1/1;
- the focused workspace-state suite passes 5/5;
- the focused GitHub enrichment suite passes 3/3;
- the focused request-logger suite passes 6/6, including the rotation
  deadlock regression;
- the Node 24 pretest build and complete lint pass after the logger changes;
- the complete `pnpm run audit` passes under Node `24.19.0` and pnpm
  `10.34.0` at `54dd2d5`, with 1,112 host tests, 69 webview tests, 394
  architecture-checked production files, host coverage of 86.85% lines,
  81.02% branches, and 84.56% functions, webview coverage of 93.96% lines,
  84.85% branches, and 92.54% functions, and successful VSIX verification;
- the follow-up filesystem-error hardening at `42ed7a8` passes pretest, the
  focused suite, and full lint;
- Graphify `0.9.48` scans 832 code files and reports 5,005 nodes and 14,084
  raw edges. The directed diagnostic reports 12,136 valid candidate edges,
  11,993 post-build edges, 1,948 dangling endpoint edges, one self-loop, no
  missing endpoints, and 142 same-endpoint relation groups;
- Repowise `0.45.0` reports 347 retained coverage files at 88.41% lines and
  80.97% branches, 8.87/10 average health, 6.70/10 hotspot health, and no
  safe-only dead-code findings. The unrestricted report contains heuristic
  low-confidence contract/configuration candidates, so no export was removed.

This checkpoint improves composition clarity and closes a real request-logger
deadlock plus fail-open filesystem classification. It does not close QA-5 or
QA-8 globally. Remaining work includes the next high-risk process/command
boundaries, repeated-run evidence, native target matrix, physical recovery,
supply-chain attribution, operational runbooks, and QA-10 release rehearsal.

## QA-5 / QA-8.82 profile-account fetch policy consolidation — 2026-08-27

This checkpoint consolidates the profile and active-window account-fetch paths
in `src/services/profileAccountFetcher.ts`, implemented in commit `831a30c`.
The two entry points previously duplicated path validation, token lookup,
missing-token handling, remote account loading, and authentication-error
classification. They now share one private policy boundary while preserving
their intentionally different missing-token messages.

The service still owns orchestration only: user-data path validation remains a
pure shared policy, credential access is consumed through `IProfileAuthReader`,
and remote account metadata is consumed through `IUserService`. No VS Code,
filesystem, or HTTP implementation is introduced into the service boundary.

Evidence for this checkpoint:

- direct tests pass 8/8, covering empty input, successful profile and active-
  window mapping, both missing-token messages, invalid paths, authentication
  expiry classification, non-authentication failures, and mixed profile
  results;
- the Node 24 pretest build and complete lint pass;
- the complete `pnpm run audit` passes under Node `24.19.0` and pnpm
  `10.34.0` at `831a30c`, with 1,112 host tests, 69 webview tests, 394
  architecture-checked production files, host coverage of 86.85% lines,
  81.02% branches, and 84.56% functions, webview coverage of 93.96% lines,
  84.85% branches, and 92.54% functions, and successful VSIX verification;
- Repowise `0.45.0` retains 347 merged coverage files at 88.55% lines and
  81.15% branches. The service improves to 9.17/10 at NLOC 124 with 91.72%
  lines and 85.71% branches. Its remaining low-cohesion, small duplication,
  and coverage-gradient findings are documented heuristics, not dependency
  direction violations;
- the synchronized Repowise index reports 5,167 nodes and 14,253 edges,
  zero unreachable files, zero unused exports in the safe summary, and 287
  unrestricted heuristic dead-code candidates. Graphify remains at 5,007
  nodes, 14,084 raw edges, and 11,993 directed post-build edges with no
  missing endpoints.

This closes the duplicated account-fetch policy and materially raises its
direct test confidence. QA-5 and QA-8 remain partial globally; process/log
tailing, broader command hotspots, native packaging, physical recovery,
supply-chain attribution, operational runbooks, and QA-10 release rehearsal
remain open.

## QA-5 / QA-7 / QA-8.86 RunSSE response serialization and finalization boundary — 2026-08-27

Commits `88d35fe` and `b241bc5` harden
`src/proxy/mitmProxyResponseHandler.ts` at the transport/application boundary.
RunSSE response chunks are now processed through a single FIFO promise chain,
and response finalization waits for that chain before finalizing and deleting
the decoder. This preserves live token updates when protobuf initialization is
slow or the transport reports response end immediately after the last chunk.
The second commit replaces long positional calls with typed lifecycle contexts
and isolates RunSSE finalization from generic response logging and summary
construction. No domain dependency was introduced into the MITM adapter.

Evidence for this checkpoint:

- the focused response-handler suite passes 2/2, including the delayed-registry
  race that previously could drop the live update;
- Node 24.19.0/pnpm 10.34.0 pretest, TypeScript compilation, and ESLint pass;
- the complete `pnpm run audit` passes at `b241bc5` with 1,157 host tests,
  88.54% lines, 82.34% branches, and 85.42% functions; webview remains 69/69
  tests with 93.96% lines, 84.85% branches, and 92.54% functions;
- architecture rules and negative fixtures pass for 400 production TypeScript
  files; localization remains 26 locales with 428 keys and documentation links
  pass for 50 Markdown files;
- Graphify `0.9.48` scans 841 code files and reports 5,061 nodes and 14,265
  raw edges; the diagnostic reports 1,966 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,299
  valid candidates, and 12,153 normalized directed edges;
- Repowise `0.45.0` reports 353 retained coverage files at 89.0% lines and
  81.8% branches, 8.89/10 average health, 6.74/10 hotspot health, and zero
  safe-only dead-code findings; the response adapter scores 6.96/10 with
  max nesting reduced to 2 and no primitive-parameter finding;
- the generated Graphify output is kept outside the repository and remains
  analysis evidence rather than a release artifact.

This closes the identified RunSSE response-processing race. It does not close
QA-5, QA-6, QA-7, or QA-8 globally; remaining work includes the next process,
storage, and release hotspots, physical recovery evidence, native target
matrix, attribution decisions, operational runbooks, and QA-10 rehearsal.

## QA-5 / QA-8.87 profile import/export command boundaries — 2026-08-27

Commit `192942a` removes the previous monolithic profile import/export command
registration boundary. `registerProfileImportExportCommands.ts` now only
assembles the two commands, while `registerProfileExportCommand.ts` and
`registerProfileImportCommand.ts` each own one VS Code adapter workflow. The
existing public registration API remains unchanged, and the profile manager,
exporter, importer, filesystem, and VS Code APIs remain at the outer adapter
boundary rather than leaking into domain policies.

The export adapter keeps profile selection, optional settings selection, save
dialog handling, and exporter invocation together as one user-facing workflow.
The import adapter keeps file selection, JSON parsing, importer validation,
warning confirmation, import execution, and result-message formatting together
as its corresponding workflow. This split reduces the registration module to a
composition concern without creating artificial abstractions across unrelated
dialogs.

Evidence for this checkpoint:

- the focused profile command suite passes 6/6, including command registration,
  current-profile/list behavior, selected-profile export, valid import, and
  invalid-file rejection without writes;
- Node 24.19.0/pnpm 10.34.0 pretest, TypeScript compilation, focused ESLint,
  architecture checks, and the complete `pnpm run audit` pass at `192942a`;
- the complete audit passes with 1,161 host tests and 69 webview tests across
  18 files, 402 architecture-checked production TypeScript files, host c8
  coverage of 88.90% lines, 82.21% branches, and 85.51% functions, and the
  existing webview coverage of 93.96% lines, 84.85% branches, and 92.54%
  functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 843 code files and reports 5,070 nodes and 14,296
  raw edges. The diagnostic reports 1,973 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,322
  valid candidate edges, and 12,176 normalized directed edges;
- Repowise `0.45.0` accepts 356 coverage entries (323 exact and 33 resolved),
  retains 355 coverage files at 89.4% lines and 81.5% branches, reports 8.90
  average health and 6.73 hotspot health, and finds no safe-only dead code;
- the composition boundary scores 10.00/10 in Repowise with NLOC 11, maximum
  CCN 1, and 100% direct line/branch coverage. The export and import adapters
  retain medium heuristic duplication and import-side coverage gradients, so
  their remaining UI cancellation, warning, and failure paths are explicit
  follow-up coverage work rather than hidden behind a facade.

This checkpoint closes the registration-module monolith and preserves the
Clean Architecture dependency direction. It does not close QA-5 or QA-8
globally; the next command hotspot is the CRUD registration boundary, followed
by the remaining process, storage, physical-recovery, packaging, attribution,
operational-runbook, and QA-10 release-rehearsal work.

## QA-5 / QA-8.88 profile CRUD command boundaries and failure-path coverage — 2026-08-27

Commits `f5e7958` and `886ea86` complete the next command-boundary slice.
The former `registerProfileCrudCommands.ts` monolith is now a composition
barrel that registers four focused VS Code adapters: add, list, delete, and
show-current. The public `registerProfileCommands` API and command identifiers
remain unchanged. Profile manager, launcher, detector, output-channel, and
confirmation-dialog concerns stay at the extension adapter boundary; no domain
or application dependency was introduced to solve a presentation concern.

The focused contracts now cover successful creation and optional launch,
duplicate rejection, creation cancellation, launch failure reporting,
deterministic profile listing including last-launched metadata, confirmed and
cancelled deletion with instance-detector forwarding, empty deletion state,
default and detected current-profile presentation, and detector failure
reporting. The barrel itself is intentionally limited to composition and does
not interpret command payloads.

Evidence for this checkpoint:

- the focused profile command suite passes 15/15;
- Node 24.19.0/pnpm 10.34.0 pretest, TypeScript compilation, focused ESLint,
  architecture checks, and the complete `pnpm run audit` pass at `886ea86`;
- the complete audit passes with 1,170 host tests and 69 webview tests across
  18 files, 406 architecture-checked production TypeScript files, host c8
  coverage of 89.28% lines, 82.26% branches, and 85.53% functions, and the
  existing webview coverage of 93.96% lines, 84.85% branches, and 92.54%
  functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 847 code files and reports 5,082 nodes and 14,341
  raw edges. The diagnostic reports 1,977 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,363
  valid candidate edges, and 12,217 normalized directed edges;
- Repowise `0.45.0` accepts 360 coverage entries (327 exact and 33 resolved),
  retains 359 coverage files at 89.7% lines and 81.6% branches, reports 8.90
  average health and 6.73 hotspot health, and finds no safe-only dead code;
- the CRUD composition barrel scores 9.65/10 with NLOC 15, maximum CCN 1,
  maximum nesting 0, and 100% direct line/branch coverage. The focused
  adapters score between 9.08/10 and 9.65/10; their remaining Repowise
  duplication and coverage-gradient markers are documented adapter-level
  follow-ups, not evidence of dependency-direction violations.

This checkpoint removes the CRUD registration monolith and adds direct
failure-path confidence without changing the command contract. It does not
close QA-5 or QA-8 globally; the next architecture hotspot is the remaining
accounts-panel coordinator, followed by process/storage hotspots, physical
recovery evidence, the native packaging matrix, supply-chain attribution,
operational runbooks, and QA-10 release rehearsal.

## QA-5 / QA-8.89 accounts-panel launch workflow boundary — 2026-08-27

Commit `cee5ba6` extracts profile-launch orchestration from
`AccountsPanelHandlers` into `AccountsPanelLaunchHandlers`. The new adapter
owns only the launch workflow: it resolves the most recent workspace when the
profile has no explicit project path, chooses between opening the current
window and invoking the profile launcher, prevents concurrent launches for
the same profile, and publishes success or failure messages through the panel
callbacks. The message dispatcher remains responsible for translating the
webview command into the workflow call; profile persistence, workspace
scanning, and process launching remain behind their existing application and
domain ports.

The extraction preserves the public panel behavior while removing the
launch-specific state and routing branches from the main panel coordinator.
It does not move UI message interpretation into the domain, and it does not
introduce a second profile-launch implementation. The remaining coordinator
still contains unrelated presentation workflows, which are deliberately
tracked as the next refactoring candidates rather than hidden by an oversized
facade.

Evidence for this checkpoint:

- the direct launch-workflow suite passes 3/3 contracts, covering successful
  recent-workspace resolution and refresh notifications, failure without a
  false refresh, and same-profile single-flight deduplication; the existing
  accounts-panel launch and proxy/storage suites remain green;
- Node 24.19.0/pnpm 10.34.0 pretest, TypeScript compilation, focused ESLint,
  architecture checks, and the complete `pnpm run audit` pass at `cee5ba6`;
- the complete audit passes with 1,173 host tests and 69 webview tests across
  18 files, 407 architecture-checked production TypeScript files, and host
  c8 coverage of 89.32% lines, 82.31% branches, and 85.56% functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 849 code files and reports 5,093 nodes and 14,387
  raw edges. The diagnostic reports 1,982 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,405
  valid candidate edges, and 12,259 normalized directed edges;
- Repowise `0.45.0` accepts 361 coverage entries (328 exact and 33 resolved),
  retains 360 coverage files at 89.8% lines and 81.8% branches, reports 8.91
  average health and 6.74 hotspot health, and finds no safe-only dead code;
- Repowise scores `AccountsPanelLaunchHandlers` at 9.85/10 with NLOC 117,
  maximum CCN 6, maximum nesting 3, 100% line coverage, 88.00% branch
  coverage, and 11.86% duplication. The remaining
  `AccountsPanelHandlers` coordinator scores 4.88/10 with NLOC 244 and
  46.34% branch coverage, making it the next evidence-backed UI boundary.

This checkpoint reduces the launch responsibility of the panel coordinator
and adds direct lifecycle/routing confidence. It does not close QA-5 or QA-8
globally; remaining work includes the other accounts-panel workflows,
process/storage hotspots, physical recovery evidence, native packaging
matrix, supply-chain attribution, operational runbooks, and QA-10 release
rehearsal.

## QA-5 / QA-8.90 remaining Accounts panel presentation workflows — 2026-08-27

Commit `fa38bef` removes the remaining local presentation workflows from
`AccountsPanelHandlers`. Efficiency toggling now has a focused adapter that
validates the active profile, delegates profile-level state changes to the
existing efficiency service, and publishes the result. Suggested-profile
resolution now has a focused adapter that owns active-webview checks,
authentication reads, existing-profile lookup, response shaping, and safe
fallback publication after detection errors. The profile handler now owns the
operating-system reveal action for a stored profile directory, keeping that
profile-specific behavior with the other profile actions.

The coordinator now acts as composition and message dispatch for these
features; it no longer imports the efficiency implementation, account
suggestion policy, or VS Code reveal API for its own workflow logic. The new
dependencies are capability-shaped (`Pick` contracts or small structural
services), and all three adapters remain outside the domain layer. No webview
message contract or public command behavior changed.

Evidence for this checkpoint:

- the focused suites pass 11/11 contracts across efficiency, suggested
  profile, and profile presentation handlers, including inactive-webview
  short-circuiting, generated suggestions, existing-account notices,
  detection-error fallback, wrong-window rejection, successful result
  publication, refresh behavior, and OS reveal routing;
- the complete `pnpm run audit` passes at `fa38bef` with 1,173 host tests and
  69 webview tests across 18 files, 409 architecture-checked production
  TypeScript files, and host c8 coverage of 89.46% lines, 82.44% branches,
  and 85.57% functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 853 code files and reports 5,115 nodes and 14,454
  raw edges. The diagnostic reports 1,993 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,461
  valid candidate edges, and 12,315 normalized directed edges;
- Repowise `0.45.0` accepts 363 coverage entries (330 exact and 33 resolved),
  retains 362 coverage files at 89.9% lines and 81.9% branches, reports 8.91
  average health and 6.77 hotspot health, and finds no safe-only dead code;
- Repowise scores the remaining `AccountsPanelHandlers` coordinator at
  5.36/10 with NLOC 211, maximum CCN 2, maximum nesting 1, 86.53% line
  coverage, and 54.29% branch coverage. The extracted efficiency and
  suggested-profile adapters each score 9.65/10; the profile handler scores
  7.79/10 with 94.68% lines and 75.00% branches.

This checkpoint leaves the panel coordinator substantially smaller and makes
the presentation workflows independently testable. It does not close QA-5
or QA-8 globally; the next work should be selected from the refreshed
Repowise/Graphify hotspots and the still-open physical-recovery, packaging,
supply-chain-attribution, operational-runbook, and QA-10 rehearsal gates.

## QA-8.91 Accounts panel refresh delegation coverage — 2026-08-27

Commit `081fa74` adds direct contracts for the already-separated
`AccountsPanelDataRefresher` boundary. The tests now cover the localized error
response when the initial read model fails, the inactive-webview short-circuit
for efficiency statistics, active statistics publication, and every explicit
delegation for open workspaces, proxy status, running instances, profile
accounts, GitHub summaries, quotas, and their corresponding post operations.
The implementation is unchanged; this slice closes an observable coverage gap
without introducing a new abstraction.

Evidence for this checkpoint:

- the focused refresher suite passes 6/6 contracts;
- the complete `pnpm run audit` passes at `081fa74` with 1,173 host tests and
  69 webview tests across 18 files, 409 architecture-checked production
  TypeScript files, and host c8 coverage of 89.55% lines, 82.47% branches,
  and 85.82% functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 853 code files and reports 5,116 nodes and 14,455
  raw edges. The diagnostic reports 1,993 dangling endpoint edges, one
  self-loop, no missing endpoints, 145 same-endpoint relation groups, 12,462
  valid candidate edges, and 12,316 normalized directed edges;
- Repowise `0.45.0` accepts 363 coverage entries (330 exact and 33 resolved),
  retains 362 coverage files at 90.0% lines and 81.9% branches, reports 8.91
  average health and 6.77 hotspot health, and finds no safe-only dead code;
- Repowise reports `AccountsPanelDataRefresher` at 5.55/10 with NLOC 93,
  maximum CCN 3, maximum nesting 2, 100% line coverage, and 100% branch
  coverage. The overall performance hotspot score is 9.89/10.

This checkpoint improves evidence completeness for an existing UI read-model
boundary. QA-8 remains partial because broader process, storage, repeated-run,
and release-recovery evidence is still required.

## QA-5 / QA-8.92 profile proxy lifecycle decision boundary — 2026-08-27

Commit `789c0fc` extracts profile validation and start/ensure/restart decisions
from `ProxyManager` into the application-level
`ProxyProfileLifecycleUseCase`. The use case owns the differences between a
missing profile and a disabled proxy, resolves the complete profile set before
delegating shared-proxy startup, avoids stopping an already stopped profile,
and preserves the restart contract of stopping without restoring settings
before starting again. Logging and runtime coordination are injected as
capabilities, so the application boundary does not import VS Code, filesystem,
process, or concrete proxy infrastructure.

The public `ProxyManager` contract is unchanged. It remains a facade for
extension-facing lifecycle, status, certificate, routing, output, and traffic
operations, while the profile lifecycle decisions are now independently
testable and no longer mixed into the facade's infrastructure wiring.

Evidence for this checkpoint:

- the direct use-case suite passes 6/6 contracts, covering enabled start with
  the complete profile set, missing-versus-disabled start results, missing
  profile ensure behavior, stopped-profile restart short-circuit, running
  profile stop/start ordering, and missing-profile restart short-circuit;
- the focused facade/shared-proxy suites pass 10/10, preserving the existing
  public delegation and shared-proxy behavior;
- the complete `pnpm run audit` passes at `789c0fc` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests across 18 files,
  411 architecture-checked production TypeScript files, and host c8 coverage
  of 89.64% lines, 82.60% branches, and 85.80% functions;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 857 code files and reports 5,138 nodes and 14,515
  raw edges. The directed diagnostic reports 2,001 dangling endpoint edges,
  one self-loop, no missing endpoints, 145 same-endpoint relation groups,
  12,514 valid candidate edges, and 12,368 normalized directed edges;
- Repowise `0.45.0` accepts 365 coverage entries (332 exact and 33 resolved),
  retains 364 coverage files at 90.0% lines and 82.0% branches, reports 8.92
  average health and 6.78 hotspot health, and finds no safe-only dead code;
- Repowise reports `ProxyManager` at 4.77/10 with NLOC 289, maximum CCN 3,
  maximum nesting 1, 95.45% line coverage, and 93.62% branch coverage. The
  extracted use case scores 9.65/10 with NLOC 53, maximum CCN 3, maximum
  nesting 1, and 100% line/branch coverage.

This checkpoint removes another decision-bearing responsibility from the
proxy facade without changing its public behavior. QA-5 and QA-8 remain
partial; the next evidence-backed candidates are the remaining proxy facade
read-model/certificate/routing responsibilities and the certificate/status
hotspots, followed by physical recovery, native target-matrix, supply-chain
attribution, operational-runbook, and QA-10 release-rehearsal work.

## QA-5 / QA-6 / QA-7 / QA-8.93 certificate material generation and concurrent ensure boundary — 2026-08-27

Commit `1451171` extracts node-forge certificate-authority material generation
from `CertificateManager` into the side-effect-free
`certificateMaterialGenerator` boundary. The generator owns the certificate
shape, serial, subject, validity window, and PEM conversion; the manager keeps
filesystem reads, recovery, and persistence behind its existing infrastructure
capabilities. This preserves the established certificate paths and PEM output
while making the cryptographic-material policy independently composable.

The manager now also single-flights concurrent `ensureCaCertificate()` calls.
All callers observe the same in-flight result, and the in-flight marker is
cleared after both success and failure so a failed attempt does not permanently
poison later recovery. No public certificate-service or MITM-server contract
was changed.

Evidence for this checkpoint:

- the focused certificate and MITM boundary suites pass 21/21, including
  certificate-manager reuse, concurrent generation, certificate operations,
  certificate-service behavior, and real forwarding/runtime contracts;
- the complete `pnpm run audit` passes at `1451171` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests across 18 files;
- host c8 coverage is 89.65% lines, 82.62% branches, and 85.82% functions;
  the architecture checker passes for 412 production TypeScript files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 858 code files and reports 5,143 nodes and 14,526
  raw edges. The directed diagnostic reports 2,002 dangling endpoint edges,
  one self-loop, no missing endpoints, 144 same-endpoint relation groups,
  12,524 valid candidate edges, and 12,379 directed post-build edges;
- Repowise `0.45.0` accepts 366 coverage entries (333 exact and 33 resolved),
  retains 365 coverage files at 90.06% lines and 82.08% branches, reports
  8.92 average health and 6.78 hotspot health, and scores
  `certificateMaterialGenerator.ts` at 10.00/10 with 100% line/branch
  coverage. `certificateManager.ts` remains a tracked 4.36/10 target with
  82.52% lines and 90.91% branches because lifecycle/error-path complexity
  remains in the adapter.

This checkpoint reduces the cryptographic generation responsibility and closes
the concurrent certificate-initialization race. QA-5, QA-6, QA-7, and QA-8
remain partial: physical recovery, native privileged-command review, complete
target-matrix evidence, attribution policy, and the remaining high-risk
adapter/composition targets still require dedicated work.

## QA-6 / QA-7 / QA-8.94 certificate read-failure and partial-regeneration boundary — 2026-08-27

Commit `1a475fb` hardens the existing-certificate path after the previous
generation extraction. `CertificateManager` now evaluates both certificate and
key reads with `Promise.allSettled()`: absence is recoverable, while any
non-missing filesystem error is propagated before generation can begin. A
missing public-key file is reconstructed from a parseable certificate; a
malformed certificate remains a recoverable regeneration case, but filesystem
write failures are no longer hidden by an outer recovery catch.

This preserves the single-flight behavior introduced in `1451171` and prevents
a permission, directory, or other unexpected read failure from silently
starting a new generation and leaving a partially replaced certificate set.

Evidence for this checkpoint:

- the direct `CertificateManager` suite passes 6/6, including the regression
  contract that propagates a non-missing key read error and verifies that no
  certificate file was partially regenerated;
- the complete `pnpm run audit` passes at `1a475fb` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests across 18 files;
- host c8 coverage is 89.65% lines, 82.62% branches, and 85.82% functions;
  the architecture checker passes for 412 production TypeScript files;
- Graphify `0.9.48` scans 858 code files and reports 5,144 nodes and 14,528
  raw edges. The directed diagnostic reports 2,002 dangling endpoint edges,
  one self-loop, no missing endpoints, 144 same-endpoint relation groups,
  12,526 valid candidate edges, and 12,381 directed post-build edges;
- Repowise `0.45.0` accepts 366 coverage entries (333 exact and 33 resolved),
  retains 365 coverage files at 90.07% lines and 82.10% branches, reports
  8.92 average health and 6.78 hotspot health, and scores
  `certificateManager.ts` at 5.63/10 with 87.10% lines and 95.45% branches;
- Repowise safe-only dead-code analysis returns an empty finding set.

The certificate adapter is materially safer, but the remaining CA file writes
are still sequential and deserve a release-level crash/restart rehearsal before
QA-6 and QA-7 can be completed.

## QA-5 / QA-8.95 status-bar display-policy boundary — 2026-08-27

Commits `30c25f3` and `387bfa7` extract status-bar state decisions from the VS
Code adapter into `statusBarDisplayPolicy`. The pure policy now resolves
account selection, hidden state, profile-only display, loading, error, and
quota display—including enterprise/monthly-spend selection, translated
labels, tooltip kind, and threshold background level—without importing the
VS Code API. `StatusBarManager` retains only lifecycle state, configuration and
cache access, profile detection, MarkdownString/ThemeColor conversion, and
StatusBarItem rendering.

The initial extraction was refined after Repowise identified that the new policy
still carried the original CCN 11 decision method. Profile resolution, loading,
error, profile-only, quota, and account-selection branches are now separate
functions with the same observable outputs.

Evidence for this checkpoint:

- six direct policy contracts plus the six existing manager contracts pass
  12/12, covering every display state and the warning/error thresholds;
- the complete `pnpm run audit` passes at `387bfa7` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests across 18 files;
- host c8 coverage is 89.68% lines, 82.69% branches, and 85.87% functions;
  the architecture checker passes for 413 production TypeScript files;
- Graphify `0.9.48` scans 860 code files and reports 5,159 nodes and 14,560
  raw edges. The directed diagnostic reports 2,006 dangling endpoint edges,
  one self-loop, no missing endpoints, 144 same-endpoint relation groups,
  12,554 valid candidate edges, and 12,409 directed post-build edges;
- Repowise `0.45.0` accepts 366 coverage entries (333 exact and 33 resolved),
  retains 366 coverage files at 90.09% lines and 82.15% branches, reports
  8.92 average health and 6.79 hotspot health, and scores
  `statusBarManager.ts` at 5.79/10 with NLOC 117, max CCN 4, 91.11% lines,
  and 90.91% branches. The pure policy scores 9.15/10 with NLOC 153, max
  CCN 10, 100% lines, and 88.89% branches.

The UI decision boundary is now independently testable and infrastructure-free.
Remaining work is limited to the manager's adapter duplication/churn signal,
the proxy decoder's historical churn signal, and the open physical/release
recovery evidence.

## QA-5 / QA-8.96 binary proxy decoder boundary and recovery-path contracts — 2026-08-27

Commits `99fce13` and `01efc11` separate binary Connect/protobuf decoding from
the proxy-entry facade. `proxyDecode.ts` now owns body acquisition, RPC-path
validation, content-type selection, JSON parsing, and public result
compatibility. `proxyBinaryDecoder.ts` owns registry type resolution, raw and
Connect-framed payload candidates, decode attempts, unknown-RPC failures, and
AgentServer stream fallback. `proxyDecodedResultBuilder.ts` centralizes
sensitive redaction and insight finalization for both JSON and protobuf
results, while `proxyRpcPath.ts` keeps RPC naming/parsing reusable without a
module cycle.

The binary boundary exposes only two replaceable capabilities for tests: the
protobuf-registry loader and AgentServer-stream enricher. Production defaults
still use the existing shared registry and enrichment implementation. This
makes registry failures, malformed payloads, already-decompressed captures,
and stream fallback deterministic to test without weakening the runtime
dependency direction or changing the public `decodeProtoEntry` contract.

Evidence for this checkpoint:

- the focused compiled decoder suites pass 25/25 tests, including 15
  decode-entry/recovery contracts and the recorded RunSSE contract;
- the complete `pnpm run audit` passes at `01efc11` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests across 18 files;
- host c8 coverage is 89.71% lines, 82.68% branches, and 85.96% functions;
  the architecture checker and fixtures pass for 417 production TypeScript
  files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 864 code files and reports 5,172 nodes and 14,614
  raw edges. The directed diagnostic reports 2,007 dangling endpoint edges,
  one self-loop, no missing endpoints, 150 same-endpoint relation groups,
  12,607 valid candidate edges, and 12,456 normalized directed edges;
- Repowise `0.45.0` accepts 370 coverage entries (337 exact and 33 resolved),
  retains 369 coverage files at 90.14% lines and 82.21% branches, reports
  8.93 average health and 6.80 hotspot health, and finds no safe-only dead
  code;
- Repowise scores `proxyBinaryDecoder.ts` at 9.88/10 with NLOC 146, maximum
  CCN 7, maximum nesting 3, 96.91% lines, and 81.25% branches. The remaining
  `proxyDecode.ts` score of 6.50/10 is driven by historical churn, change
  entropy, and co-change scatter rather than residual decoder complexity.

This checkpoint closes the current decoder extraction objective. QA-5 and
QA-8 remain partial because the repository still requires broader composition
cleanup, physical disk-full/crash-restart evidence, native target-matrix
verification, supply-chain attribution decisions, operational runbooks, and
the independent final release rehearsal.

## QA-6 / QA-8.97 shared proxy-header redaction boundary — 2026-08-27

Commit `80cf1c7` removes the duplicate header-redaction implementation from
the traffic-summary presentation path and reuses the shared
`proxyRequestMetadata` policy.
The policy is case-insensitive and now covers authorization, proxy
authorization, cookie, set-cookie, x-api-key, api-key, and x-auth-token
families. This keeps traffic-summary logging and request-metadata logging on
the same privacy boundary while preserving safe headers for diagnostics.

Evidence for this checkpoint:

- the focused formatter/redaction contract passes 7/7 tests, including the
  `x-api-key` regression case;
- the complete `pnpm run audit` passes at `80cf1c7` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests;
- host c8 coverage is 89.75% lines, 82.76% branches, and 85.96% functions;
  the architecture checker passes for 417 production TypeScript files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 864 code files and reports 5,170 nodes and 14,615
  raw edges. The directed diagnostic reports 2,007 dangling endpoint edges,
  one self-loop, no missing endpoints, 151 same-endpoint relation groups,
  12,608 valid candidate edges, and 12,456 normalized directed edges;
- Repowise `0.45.0` accepts 370 coverage entries (337 exact and 33 resolved),
  retains 369 coverage files at 90.14% lines and 82.20% branches, reports
  8.93 average / 6.80 hotspot health, and finds no safe-only dead code;
- the pre-extraction traffic-formatting boundary report was 6.33/10 with NLOC
  209, maximum CCN 8, maximum nesting 3, 95.69% lines, and 76.70% branches;
  after the extraction, the UI formatter reports 9.81/10 and the proxy summary
  boundary reports 9.85/10.

This closes the current duplicate-redaction path. QA-6 and QA-8 remain
partial because physical disk-full/crash-restart evidence, native target
matrix verification, supply-chain attribution decisions, operational
runbooks, and the independent final release rehearsal are still open.

## QA-5 / QA-8.98 traffic-summary domain and UI-presentation boundaries — 2026-08-27

Commit `3f553ff` separates proxy traffic summary construction from live UI
line rendering. `proxyTrafficSummary.ts` now owns request metadata extraction,
endpoint normalization, body-size classification, and conversion from a JSONL
entry to the domain `ProxyTrafficSummary`. The former mixed formatter path was
moved to `src/ui/presentation/proxyTrafficLineFormatter.ts`, where it owns
human-readable output and token, cost, context, and agent hints. Production
proxy adapters consume the summary boundary directly, while the UI consumes
the presentation boundary directly; no proxy-layer module imports UI code.

Evidence for this checkpoint:

- the focused summary, presentation, and metadata contracts pass 12/12 tests;
- the complete `pnpm run audit` passes at `3f553ff` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests;
- host c8 coverage is 89.75% lines, 82.76% branches, and 85.96% functions;
  the architecture checker and fixtures pass for 418 production TypeScript
  files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 865 code files and reports 5,172 nodes and 14,618
  raw edges. The directed diagnostic reports 2,007 dangling endpoint edges,
  one self-loop, no missing endpoints, 150 same-endpoint relation groups,
  12,611 valid candidate edges, and 12,460 normalized directed edges;
- Repowise `0.45.0` accepts 371 coverage entries (338 exact and 33 resolved),
  retains 370 coverage files at 90.1% lines and 82.2% branches, reports
  8.93 average / 6.80 hotspot health, and finds no safe-only dead code;
- Repowise scores `proxyTrafficLineFormatter.ts` at 9.81/10 with NLOC 112,
  maximum CCN 8, maximum nesting 2, 95.28% lines, and 70.00% branches;
  `proxyTrafficSummary.ts` scores 9.85/10 with NLOC 99, maximum CCN 7,
  maximum nesting 3, 96.26% lines, and 86.05% branches.

This closes the traffic-summary/presentation extraction objective. QA-5 and
QA-8 remain partial because broader composition cleanup, physical
disk-full/crash-restart evidence, native target-matrix verification,
supply-chain attribution decisions, operational runbooks, and the independent
final release rehearsal are still open.

## QA-5 / QA-8.99 proxy-settings application operation deduplication — 2026-08-27

Commit `ae74593` centralizes per-profile proxy application and VS Code
synchronization inside `ProxySettingsService`. The all-profiles and
running-profiles flows now share the same failure-tolerant operation while
preserving running-profile filtering and the existing log messages. Restore and
backup-read responsibilities remain unchanged; the remaining low-cohesion
finding is intentionally tracked as an open follow-up rather than being hidden
by a compatibility facade.

Evidence for this checkpoint:

- the focused `ProxySettingsService` suite passes 8/8 tests;
- the complete `pnpm run audit` passes at `ae74593` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests;
- host c8 coverage is 89.78% lines, 82.79% branches, and 85.97% functions;
  the architecture checker passes for 418 production TypeScript files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- Graphify `0.9.48` scans 865 code files and reports 5,174 nodes and 14,624
  raw edges. Its diagnostic reports 2,007 dangling endpoint edges, one
  self-loop, no missing endpoints, 150 same-endpoint relation groups, 12,616
  valid candidate edges, and 12,465 normalized directed edges;
- Repowise `0.45.0` accepts 371 coverage entries (338 exact and 33 resolved),
  retains 370 coverage files at 90.2% lines and 82.2% branches, reports
  8.93 average / 6.78 hotspot health, and finds no safe-only dead code;
- Repowise reports `proxySettingsService.ts` at 5.38/10 with NLOC 123,
  maximum CCN 6, maximum nesting 4, 87.67% lines, and 77.27% branches;
  remaining findings are low cohesion and nested complexity, plus coverage and
  historical signals.

This closes the duplicate application-operation path. QA-5 and QA-8 remain
partial because the service's responsibility split, broader composition
cleanup, physical disk-full/crash-restart evidence, native target-matrix
verification, supply-chain attribution decisions, operational runbooks, and
the independent final release rehearsal are still open.

## QA-5 / QA-8.100 proxy-settings responsibility separation — 2026-08-27

Commit `cc1f4fb` completes the responsibility split identified by the previous
deduplication checkpoint. Profile-wide application, restoration, and backup
inspection now live in separate application services. The active-window VS
Code side effect is behind the domain port
`IProxyWindowConfiguration`, implemented by the `VscodeProxyWindowConfiguration`
adapter. `ProxyManager` now consumes `IProxySettingsRestorer`, while the
Accounts panel consumes `IProxySettingsBackupReader`; the small
`ProxySettingsService` facade remains an intentional compatibility boundary for
the existing aggregate API and is separately tracked for possible removal.

Evidence for this checkpoint:

- the focused proxy-settings and related integration suites pass 15/15 tests;
- the complete `pnpm run audit` passes at `cc1f4fb` under Node 24.19.0 and
  pnpm 10.34.0, with 1,193 host tests and 69 webview tests;
- host c8 coverage is 89.78% lines, 82.79% branches, and 85.97% functions;
  the architecture checker and fixtures pass for 424 production TypeScript
  files;
- localization remains at 26 locales with 428 keys, documentation links pass
  for 50 Markdown files, and the selected VSIX contents verify successfully;
- clean-archive Graphify `0.9.48` scans 871 code files and reports 5,213 nodes
  and 15,485 raw edges, with 1,836 dangling endpoint edges, one self-loop, no
  missing endpoints, 157 same-endpoint relation groups, 13,649 valid candidate
  edges, and 13,491 normalized directed edges;
- Repowise `0.45.0` accepts 375 coverage entries (342 exact and 33 resolved),
  retains 374 coverage files at 90.2% lines and 82.2% branches, reports
  8.93 average / 6.82 hotspot health, and finds no safe-only dead code;
- the extracted boundaries score `proxySettingsApplicationService.ts` 9.29/10,
  `proxySettingsRestorationService.ts` 9.74/10, and
  `proxySettingsBackupReader.ts` 9.39/10. Their remaining findings are
  coverage gradients; the compatibility facade reports 4.95/10 with LCOM4=4,
  so its eventual removal remains an explicit follow-up.

This closes the responsibility extraction objective. QA-5 and QA-8 remain
partial because the compatibility-facade decision, adapter coverage,
broader composition cleanup, physical disk-full/crash-restart evidence,
native target-matrix verification, supply-chain attribution decisions,
operational runbooks, and the independent final release rehearsal are still
open.

## QA-8.101 proxy-window adapter coverage — 2026-08-27

Commit `4ddfb2f` adds direct tests for the VS Code proxy-window adapter and its
underlying configuration operations. The tests verify every temporary setting
written by synchronization, every setting removed during cleanup, and the use
of the global configuration target. This closes the previously untested
adapter path without weakening the application/domain boundary.

Evidence for this checkpoint:

- the focused adapter suite passes 3/3 tests;
- the complete `pnpm run audit` passes at `4ddfb2f` under Node 24.19.0 and
  pnpm 10.34.0, with 1,209 host tests and 69 webview tests;
- host c8 coverage is 89.86% lines, 82.82% branches, and 85.88% functions;
  the architecture checker and fixtures pass for 424 production TypeScript
  files;
- `syncProxyVscodeConfiguration.ts` and
  `vscodeProxyWindowConfiguration.ts` both report 100% line and branch
  coverage in the audit output;
- clean-archive Graphify `0.9.48` scans 872 code files and reports 5,216 nodes
  and 15,495 raw edges, with 1,839 dangling endpoint edges, one self-loop, no
  missing endpoints, 157 same-endpoint relation groups, 13,656 valid candidate
  edges, and 13,498 normalized directed edges;
- Repowise `0.45.0` reports 374 retained coverage files at 90.2% lines and
  82.3% branches, 8.94 average / 6.82 hotspot health, and no safe-only
  dead-code findings; both adapter files score 10.00/10 with no findings.

This closes the proxy-window adapter coverage objective. The remaining QA-5
and QA-8 work is the explicit compatibility-facade decision, broader
composition cleanup, and the release/security evidence already listed in the
current execution register.

## QA-5 / QA-8.102 proxy-settings facade removal and direct application wiring — 2026-08-27

Commit `3f1f58f` removes the unused `ProxySettingsService` compatibility facade
from production. The extension composition root now wires the dedicated
application, restoration, backup-reader, and VS Code window-configuration
boundaries directly. `ProxyManager` consumes the restoration port and the
single-profile application capability through its composition boundary, while
the profile-settings adapter remains the fallback used by isolated alternate
compositions and tests. This removes the Repowise low-cohesion facade hotspot
without merging the separate application responsibilities again.

Evidence for this checkpoint:

- the focused proxy-settings, integration, and ProxyManager suites pass 17/17;
- the complete `pnpm run audit` passes at `3f1f58f` under Node 24.19.0 and
  pnpm 10.34.0, with 1,210 host tests and 69 webview tests;
- exact host c8 coverage is 89.85% lines, 82.81% branches, and 86.04%
  functions; webview coverage is 93.96% lines, 84.85% branches, and 92.54%
  functions;
- the architecture checker and fixtures pass for 423 production TypeScript
  files; the full compiled host suite also passes independently under Node
  24.19.0, while default-runtime execution is excluded because the SQLite
  native binding is ABI-specific;
- clean-archive Graphify `0.9.48` scans 871 code files and reports 5,210 nodes
  and 15,476 raw edges, with 1,839 dangling endpoint edges, one self-loop, no
  missing endpoints, 156 same-endpoint relation groups, 13,637 valid candidate
  edges, and 13,480 normalized directed edges;
- combined host/webview Repowise ingestion accepts 374 entries (341 exact and
  33 resolved), retains 373 coverage files at 90.23% lines and 82.23%
  branches, reports 8.94 average / 6.82 hotspot health, and finds no
  safe-only dead code;
- Repowise reports `proxySettingsApplicationService.ts` at 9.31/10,
  `proxySettingsRestorationService.ts` at 9.74/10, and
  `proxySettingsBackupReader.ts` at 9.39/10. The former facade is no longer
  indexed as a production target.

This closes the proxy-settings compatibility-facade objective. QA-5 and QA-8
remain partial because broader composition cleanup, physical disk-full and
crash-restart evidence, native target-matrix verification, supply-chain
attribution decisions, operational runbooks, and the independent final release
rehearsal remain open.

## QA-5 / QA-8.103 efficiency persistence read-adapter separation — 2026-08-27

Commit `b90554b` separates the efficiency database read model into three
focused persistence adapters while preserving the public `EfficiencyDatabase`
facade and the existing SQLite schema. Aggregate profile/repository/branch
projections now live in `efficiencyAggregateReader.ts`, filtered event reads in
`efficiencyEventReader.ts`, and quota range/trend projections in
`efficiencyQuotaReader.ts`. Each adapter receives the existing
`SqliteExecutor` at the composition boundary; no adapter constructs a database,
depends on VS Code, or crosses into the domain/application layers.

The public facade still owns database initialization, migrations, writes,
corrupted-artifact preservation, and compatibility-shaped read methods. This
keeps the persistence policy in one infrastructure entry point while avoiding
one reader object with unrelated aggregate, event, and quota responsibilities.
The adapters are functional factories with explicit interfaces so the boundary
is injectable without introducing artificial class state or cohesion warnings.

Evidence for the slice:

- the focused persistence suites pass 7/7 tests, including aggregate stats,
  repository and branch filters, quota phases, quota ranges, quota trends,
  cleanup, and corrupted-database sidecar preservation;
- the complete `pnpm run audit` passes at `b90554b` under Node `24.19.0` and
  pnpm `10.34.0`, with 1,210 host tests and 69 webview tests;
- exact host c8 coverage is 90.13% lines, 83.17% branches, and 86.36%
  functions; webview coverage is 93.96% lines, 84.85% branches, and 92.55%
  functions;
- the architecture checker and fixtures pass for 426 production TypeScript
  files; lint and TypeScript compilation also pass;
- clean-tree Graphify `0.9.48` scans 874 code files and reports 5,242 nodes,
  14,808 raw edges, and 12,642 normalized directed post-build edges. Its
  diagnostic reports 2,014 dangling endpoints, one self-loop, no missing
  endpoints, 156 same-endpoint groups, and 12,794 valid candidate edges;
- combined host/webview Repowise ingestion accepts 377 entries (344 exact and
  33 resolved), retains 376 coverage files at 90.48% lines and 82.73%
  branches, and reports 8.94 average / 6.79 hotspot health;
- Repowise reports the aggregate reader at 9.52/10, the event reader at
  9.85/10, and the quota reader at 10.00/10. The database facade remains at
  5.73/10 because of its historical churn, coverage gradient, and low-impact
  error-handling/duplication signals; no cohesion finding remains on the
  separated readers;
- Repowise safe-only dead-code analysis returns no findings and its historical
  security scan returns zero findings across 483 commits, 6,100 blobs, and
  3,170 files.

This checkpoint improves the persistence boundary but does not close QA-5 or
QA-8 globally. Remaining work includes the documented composition/refactoring
targets, repeated static-analysis evidence, native target-matrix execution,
physical recovery/security testing, supply-chain attribution decisions,
operational runbooks, and the independent release rehearsal.

## QA-5 / QA-8.104 agent stream framing helper consolidation — 2026-08-27

Commit `7bca506` consolidates the repeated gzip and Connect-framing candidate
walk used by agent stream decoding. Direct `AgentServerMessage` decoding and
`HealthResponse.payload` decoding now share one pure helper that tries raw and
gzip candidates, iterates supported Connect payload candidates, isolates decode
failures, and returns the first successful result. The semantic decoders remain
separate, so the `HealthResponse` unwrapping policy is not hidden inside the
generic framing mechanism.

Evidence for the slice:

- the focused decoder, proxy-decoder, and streaming-decoder suites pass 37/37
  tests, including direct payloads, `HealthResponse` wrapping, multi-frame
  streams, partial frames, finalization, and a new gzip-compressed Connect
  regression contract;
- the complete `pnpm run audit` passes at `7bca506` under Node `24.19.0` and
  pnpm `10.34.0`, with 1,210 host tests and 69 webview tests;
- exact host c8 coverage is 90.14% lines, 83.22% branches, and 86.36%
  functions; webview coverage is 93.96% lines, 84.85% branches, and 92.55%
  functions;
- the architecture checker and fixtures pass for 426 production TypeScript
  files; lint, TypeScript compilation, localization, documentation links,
  supply-chain checks, and VSIX verification also pass;
- clean-tree Graphify `0.9.48` scans 874 code files and reports 5,243 nodes,
  14,812 raw edges, and 12,645 normalized directed post-build edges. Its
  diagnostic reports 2,015 dangling endpoints, one self-loop, no missing
  endpoints, 156 same-endpoint groups, and 12,797 valid candidate edges;
- combined host/webview Repowise ingestion retains 376 coverage files at
  90.49% lines and 82.78% branches, with 8.94 average / 6.80 hotspot health;
- Repowise reports `agentStreamDecode.ts` at 7.26/10 with NLOC 182, maximum
  CCN 7, maximum nesting 4, 92.20% line coverage, and 77.08% branch coverage.
  The remaining medium duplication signal is cross-tooling overlap with the
  independent `scripts/lib/agent-text-extract.mjs` parser and is intentionally
  not merged across runtime and analysis layers; two defensive catches and
  recent churn remain documented signals.

This checkpoint removes the runtime decoder's duplicated framing algorithm but
does not close QA-5 or QA-8 globally. Remaining work includes the documented
webview/application composition targets, repeated static-analysis evidence,
native target-matrix execution, physical recovery/security testing,
supply-chain attribution decisions, operational runbooks, and the independent
release rehearsal.

## QA-5 / QA-8.105 webview profile-action and proxy relationship boundaries — 2026-08-27

Commits `238f5c2` and `cafd173` split the webview profile-action facade into
typed bridge and form hooks, then centralized conversation and conversation-
group relationship merging for Bidi and agent-stream proxy insights. The
profile-action facade now only composes application-facing hooks; host command
dispatch remains in the bridge hook, while form state and persisted webview
state remain in the form hook. The proxy relationship policy is pure and is
shared without combining the distinct Bidi and stream decoders.

Evidence for this checkpoint:

- the complete `pnpm run audit` passes at `cafd173` under Node `24.19.0` and
  pnpm `10.34.0`, with 1,210 host tests and 70 webview tests across 18 files;
- exact host c8 coverage is 90.16% lines, 83.22% branches, and 86.32%
  functions; webview coverage is 94.19% lines, 85.40% branches, and 92.73%
  functions;
- the architecture checker and fixtures pass for 426 production TypeScript
  files; lint, TypeScript compilation, localization, documentation links,
  supply-chain checks, and VSIX verification also pass;
- the focused proxy insight suite passes 4/4 tests, and the webview suite
  passes 70/70 tests with 100% line and branch coverage for each extracted
  profile-action hook through App integration;
- clean temporary code-only Graphify `0.9.48` scans 875 code files and reports
  5,257 nodes, 14,861 raw edges, and 12,685 normalized directed post-build
  edges. Its diagnostic reports 2,019 dangling endpoints, one self-loop, no
  missing endpoints, 151 same-endpoint groups, and 12,842 valid candidate
  edges;
- combined host/webview Repowise ingestion accepts 380 entries (344 exact and
  36 resolved), retains 379 coverage files at 90.54% lines and 82.86% branches,
  and reports 8.94 average / 6.81 hotspot health;
- Repowise reports `useAppProfileActions.ts` at 10.00/10, the bridge hook at
  9.65/10, and the form hook at 9.62/10. The shared relationship policy
  removes a real duplicate block; `proxyInsightEnricher.ts` remains at
  8.38/10 with coverage and prior-defect signals documented for follow-up.

This checkpoint improves webview/application cohesion and removes duplicated
proxy relationship policy. QA-5 and QA-8 remain partial because broader
composition targets, repeated static-analysis evidence, native target-matrix
execution, physical recovery/security testing, supply-chain attribution
decisions, operational runbooks, and the independent release rehearsal remain
open.

## QA-5 / QA-8.106 extension window initialization boundary — 2026-08-27

Commit `895da24` extracts current-window initialization from the activation
orchestrator. Profile detection, attachment to an existing enabled proxy, and
accounts-panel auto-open logging now live behind a focused composition
workflow, while the caller retains profile-manager/efficiency startup and the
stale-activation cleanup guard around shared-proxy startup.

Evidence for this checkpoint:

- the complete `pnpm run audit` passes at `895da24` under Node `24.19.0` and
  pnpm `10.34.0`, with 1,215 host tests and 70 webview tests;
- exact host c8 coverage is 90.37% lines, 83.20% branches, and 86.43%
  functions; webview coverage is 94.19% lines, 85.40% branches, and 92.73%
  functions;
- the architecture checker and fixtures pass for 426 production TypeScript
  files; lint, TypeScript compilation, localization, documentation links,
  supply-chain checks, and VSIX verification also pass;
- the new initialization suite passes 3/3 contracts for an unassigned window,
  an enabled active profile, and stale activation after shared-proxy startup;
- clean temporary code-only Graphify `0.9.48` scans 876 code files and reports
  5,262 nodes, 15,645 raw edges, and 13,635 normalized post-build edges. Its
  diagnostic reports 1,848 dangling endpoints, one self-loop, no missing
  endpoints, 156 same-endpoint groups, and 13,797 valid candidate edges;
- Repowise reports `src/composition/extensionInitialization.ts` at 9.53/10
  with NLOC 97, maximum CCN 8, maximum nesting 2, 88.14% line coverage, and
  60.00% branch coverage; its remaining signal is a coverage gradient.

This checkpoint improves the activation composition boundary and adds direct
failure/lifecycle characterization. QA-5 and QA-8 remain partial because
broader composition targets, repeated static-analysis evidence, native
target-matrix execution, physical recovery/security testing, supply-chain
attribution decisions, operational runbooks, and the independent release
rehearsal remain open.
...
| 2026-08-27 | QA-5/QA-8.79 profile workspace application boundary | 1f3071c, 0f96c1b | Moved profile workspace orchestration into `application/services`, introduced the `IWorkspaceScanner` domain port, kept filesystem scanning in the profiles adapter, added 3/3 direct contracts, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 389 architecture-checked production files; Repowise reports the service at 10.00/10 with 100% line/branch coverage and zero unused exports |
| 2026-08-27 | QA-5/QA-8.80 accounts-panel proxy-state boundary | f11f8e6, 63411e0 | Isolated proxy read/status/temporary-profile composition behind `AccountsPanelProxyStateCoordinator`, introduced the narrow `IProxySettingsBackupReader` port, added 5 direct coordinator contracts, passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,112 host and 69 webview tests, 392 architecture-checked production files, Repowise 8.86 average / 6.71 hotspot health, coordinator health 9.46/10, and Graphify 4,976 nodes/13,955 raw edges/11,888 directed post-build edges |
| 2026-08-27 | QA-5/QA-8.81 accounts-panel composition and request-logger reliability | da4e248, bc1995b, a8d4099, 0764a2a, 7ed629a, 87a896f, 54dd2d5, 42ed7a8 | Isolated initial-data, workspace-state, and GitHub enrichment contracts; replaced the 15-argument panel constructor with a named dependency object; fixed request-logger rotation deadlock and non-ENOENT filesystem swallowing; complete audit at 54dd2d5 with 1,112 host and 69 webview tests, 394 architecture-checked production files, focused logger/pretest/lint evidence at 42ed7a8, Graphify 5,005 nodes/14,084 raw edges/11,993 directed post-build edges, and Repowise 8.87 average / 6.70 hotspot health with safe-only dead-code empty |
| 2026-08-27 | QA-5/QA-7/QA-8.82 transactional MITM startup and composition boundary | 601056f, 453e31a | Made partial MITM startup cleanup explicit for certificate, logger, and listener failures; preserved single-flight start/stop ordering and retry behavior; extracted proto-registry initialization and handler configuration into private composition helpers while keeping the server as the runtime composition root; lifecycle suite passes 4/4 and the real forwarding contract passes 1/1 with loopback permissions; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,112 host and 69 webview tests and 394 architecture-checked production files; Repowise retains 347 coverage files at 88.56% lines and 81.17% branches, reports 8.87 average / 6.72 hotspot health and `mitmProxyServer.ts` at 6.19/10 with NLOC 238, max CCN 4, max nesting 3, 92.31% lines and 75.00% branches; Graphify reports 5,009 nodes, 14,089 raw edges, 11,999 directed post-build edges, 1,948 dangling endpoints, one self-loop, no missing endpoints, and 141 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-7/QA-8.83 proxy JSONL lifecycle and boundary extraction | 3b1cdfa, a6b1f16 | Separated file discovery/offset reads and JSONL entry processing from lifecycle orchestration; added lifecycle generations, single-flight startup, stale-callback suppression, recoverable serialized reads, deterministic rotation, and missing/truncated-file recovery; added 13 direct contracts covering partial lines, request/response duration, malformed/error records, inactive generations, deterministic discovery, offsets, truncation, and missing files; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,149 host and 69 webview tests and 396 architecture-checked production files; host c8 is 88.10% lines/81.77% branches/85.31% functions; Repowise retains 349 coverage files at 88.65% lines and 81.34% branches, reports 8.87 average / 6.71 hotspot health, `proxyLogTailer.ts` at 6.54/10 with NLOC 200, max CCN 6, max nesting 3, 87.18% lines and 78.00% branches, `proxyLogFileReader.ts` at 9.78/10, and `proxyLogEntryProcessor.ts` at 9.37/10; Graphify reports 5,033 nodes, 14,167 raw edges, 12,065 directed post-build edges, 1,959 dangling endpoints, one self-loop, no missing endpoints, and 142 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.84 proxy log storage and agent insight policy boundaries | 817287c, b452540, 86eeea4, 8d8a225 | Extracted JSONL stream/rotation/retention into `ProxyLogStorage` behind the `RequestLogger` compatibility facade; split agent insight support, usage/cost validation, and request/relationship extraction into cohesive policies; added storage retention and agent cost/prewarm/subagent/workspace contracts; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,156 host and 69 webview tests and 400 architecture-checked production files; host c8 is 88.33% lines/82.19% branches/85.34% functions; Repowise retains 353 coverage files at 88.9% lines and 81.5% branches, reports 8.89 average / 6.73 hotspot health, usage extraction at 9.81/10, and relationship extraction at 9.12/10; Graphify reports 5,041 nodes, 14,183 raw edges, 12,076 directed post-build edges, 1,964 dangling endpoints, one self-loop, no missing endpoints, and 142 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.84 proxy log storage and agent insight policy boundaries | 817287c, b452540, 86eeea4, 8d8a225 | Extracted JSONL stream/rotation/retention into `ProxyLogStorage` behind the `RequestLogger` compatibility facade; split agent insight support, usage/cost validation, and request/relationship extraction into cohesive policies; added storage retention and agent cost/prewarm/subagent/workspace contracts; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,156 host and 69 webview tests and 400 architecture-checked production files; host c8 is 88.33% lines/82.19% branches/85.34% functions; Repowise retains 353 coverage files at 88.9% lines and 81.5% branches, reports 8.89 average / 6.73 hotspot health, usage extraction at 9.81/10, and relationship extraction at 9.12/10; Graphify reports 5,041 nodes, 14,183 raw edges, 12,076 directed post-build edges, 1,964 dangling endpoints, one self-loop, no missing endpoints, and 142 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.85 conversation-context policy boundary | b9703a6 | Extracted file-path and message-context normalization into a focused policy, reduced nested traversal, added malformed/empty context contracts, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,157 host and 69 webview tests and 400 architecture-checked production files; host c8 is 88.35% lines/82.30% branches/85.36% functions; Repowise retains 353 coverage files at 88.9% lines and 81.6% branches, reports 8.89 average / 6.73 hotspot health, context extraction at 9.85/10, and Graphify reports 5,057 nodes, 14,250 raw edges, 12,138 directed post-build edges, 1,966 dangling endpoints, one self-loop, no missing endpoints, and 145 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-7/QA-8.86 RunSSE response serialization and finalization boundary | 88d35fe, b241bc5 | Serialized RunSSE chunk processing and made response finalization await delayed decoder initialization, then replaced long positional lifecycle calls with typed contexts; focused response-handler suite passes 2/2, complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,157 host and 69 webview tests, 400 architecture-checked production files, and host c8 at 88.54% lines/82.34% branches/85.42% functions; Repowise retains 353 coverage files at 89.0% lines and 81.8% branches, reports 8.89 average / 6.74 hotspot health and adapter health 6.96/10; Graphify reports 5,061 nodes, 14,265 raw edges, 12,153 normalized directed edges, 1,966 dangling endpoints, one self-loop, no missing endpoints, and 145 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.87 profile import/export command boundaries | 192942a | Split the profile import/export registration monolith into separate VS Code adapters and a small composition function; focused profile command suite passes 6/6, complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,161 host and 69 webview tests, 402 architecture-checked production files, and host c8 at 88.90% lines/82.21% branches/85.51% functions; Repowise retains 355 coverage files at 89.4% lines and 81.5% branches, reports 8.90 average / 6.73 hotspot health, and gives the composition boundary 10.00/10; Graphify reports 5,070 nodes, 14,296 raw edges, 12,176 normalized directed edges, 1,973 dangling endpoints, one self-loop, no missing endpoints, and 145 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.88 profile CRUD command boundaries and failure-path coverage | f5e7958, 886ea86 | Split the CRUD registration monolith into add/list/delete/current VS Code adapters and added cancellation, validation, confirmation, empty-state, and failure-path contracts; focused profile command suite passes 15/15, complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,170 host and 69 webview tests, 406 architecture-checked production files, and host c8 at 89.28% lines/82.26% branches/85.53% functions; Repowise retains 359 coverage files at 89.7% lines and 81.6% branches, reports 8.90 average / 6.73 hotspot health, and gives the composition barrel 9.65/10; Graphify reports 5,082 nodes, 14,341 raw edges, 12,217 normalized directed edges, 1,977 dangling endpoints, one self-loop, no missing endpoints, and 145 same-endpoint relation groups; safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.89 accounts-panel launch workflow boundary | cee5ba6 | Extracted launch routing, recent-workspace resolution, same-profile single-flight protection, and launch result publication into `AccountsPanelLaunchHandlers`; direct launch suite 3/3; complete audit with 1,173 host and 69 webview tests, 407 architecture-checked production files, host c8 89.32%/82.31%/85.56%, Graphify 5,093 nodes/14,387 raw edges/12,259 normalized directed edges, Repowise 8.91 average / 6.74 hotspot health and 360 retained coverage files at 89.8%/81.8% |
| 2026-08-27 | QA-5/QA-8.90 remaining Accounts panel presentation workflows | fa38bef | Extracted efficiency toggle, suggested-profile detection, and profile-directory reveal behavior into focused UI adapters; focused suites 11/11; complete audit with 1,173 host and 69 webview tests, 409 architecture-checked production files, host c8 89.46%/82.44%/85.57%, Graphify 5,115 nodes/14,454 raw edges/12,315 normalized directed edges, Repowise 8.91 average / 6.77 hotspot health and 362 retained coverage files at 89.9%/81.9% |
| 2026-08-27 | QA-8.91 Accounts panel refresh delegation coverage | 081fa74 | Added direct error, inactive-webview, efficiency-stats, and delegation contracts for `AccountsPanelDataRefresher`; focused suite 6/6; complete audit with 1,173 host and 69 webview tests, 409 architecture-checked production files, host c8 89.55%/82.47%/85.82%, Graphify 5,116 nodes/14,455 raw edges/12,316 normalized directed edges, Repowise 8.91 average / 6.77 hotspot health and 362 retained coverage files at 90.0%/81.9% |
| 2026-08-27 | QA-5/QA-8.92 profile proxy lifecycle decision boundary | 789c0fc | Extracted profile start/ensure/restart validation into `ProxyProfileLifecycleUseCase`; direct suite 6/6 and focused facade/shared-proxy suites 10/10; complete audit with 1,193 host and 69 webview tests, 411 architecture-checked production files, host c8 89.64%/82.60%/85.80%, Graphify 5,138 nodes/14,515 raw edges/12,368 normalized directed edges, Repowise 8.92 average / 6.78 hotspot health, `ProxyManager` at 4.77/10 and 364 retained coverage files at 90.0%/82.0% |
| 2026-08-27 | QA-5/QA-6/QA-7/QA-8.93 certificate material generation and concurrent ensure boundary | 1451171 | Extracted node-forge material generation into a side-effect-free boundary and made CA initialization single-flight with retry-safe failure cleanup; focused certificate/MITM suites pass 21/21, complete audit passes with 1,193 host and 69 webview tests, 412 architecture-checked production files, host c8 89.65%/82.62%/85.82%, Graphify 5,143 nodes/14,526 raw edges/12,379 directed post-build edges with 2,002 dangling endpoints and 144 same-endpoint groups, and Repowise 8.92 average / 6.78 hotspot health with 365 retained coverage files at 90.06%/82.08% |
| 2026-08-27 | QA-6/QA-7/QA-8.94 certificate read-failure and partial-regeneration boundary | 1a475fb | Propagated non-missing certificate/key read failures before generation, preserved recoverable absence/malformed-certificate behavior, and added a no-partial-regeneration regression contract; direct CertificateManager suite 6/6 and complete audit pass with 1,193 host and 69 webview tests, 412 architecture-checked production files, host c8 89.65%/82.62%/85.82%, Graphify 5,144 nodes/14,528 raw edges/12,381 directed post-build edges, and Repowise 8.92 average / 6.78 hotspot health with 365 retained coverage files at 90.07%/82.10% |
| 2026-08-27 | QA-5/QA-8.95 status-bar display-policy boundary | 30c25f3, 387bfa7 | Extracted VS Code-free status-bar display decisions, then split the remaining CCN 11 method into focused branches; direct policy/manager suites pass 12/12, complete audit passes with 1,193 host and 69 webview tests, 413 architecture-checked production files, host c8 89.68%/82.69%/85.87%, Graphify 5,159 nodes/14,560 raw edges/12,409 directed post-build edges, and Repowise 8.92 average / 6.79 hotspot health with 366 retained coverage files at 90.09%/82.15% |
| 2026-08-27 | QA-5/QA-8.96 binary proxy decoder boundary and recovery-path contracts | 99fce13, 01efc11 | Split RPC-path naming, binary type resolution, Connect payload selection, decode failure handling, and AgentServer fallback from the proxy-entry facade; added injectable registry/enrichment capabilities and 5 direct recovery contracts; focused decoder suites pass 25/25, complete audit passes with 1,193 host and 69 webview tests, 417 architecture-checked production files, host c8 89.71%/82.68%/85.96%, Graphify 5,172 nodes/14,614 raw edges/12,456 directed post-build edges, and Repowise 8.93 average / 6.80 hotspot health with 369 retained coverage files at 90.14%/82.21% |
| 2026-08-27 | QA-6/QA-8.97 shared proxy-header redaction boundary | 80cf1c7 | Removed duplicate formatter redaction in favor of the shared case-insensitive metadata policy, added the `x-api-key` regression contract, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,193 host and 69 webview tests, 417 architecture-checked production files, host c8 89.75%/82.76%/85.96%, Graphify 5,170 nodes/14,615 raw edges/12,456 directed post-build edges, Repowise 8.93 average / 6.80 hotspot health with 369 retained coverage files at 90.14%/82.20%, and safe-only dead-code analysis empty |
| 2026-08-27 | QA-5/QA-8.98 traffic-summary domain and UI-presentation boundaries | 3f553ff | Separated proxy JSONL/domain summary construction from UI line rendering, moved presentation formatting to `src/ui/presentation`, updated production imports to the narrow boundaries, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,193 host and 69 webview tests, 418 architecture-checked production files, host c8 89.75%/82.76%/85.96%, Graphify 5,172 nodes/14,618 raw edges/12,460 directed post-build edges, Repowise 8.93 average / 6.80 hotspot health with 370 retained coverage files at 90.1%/82.2%, formatter health 9.81/10, summary health 9.85/10, and safe-only dead-code analysis empty |
| 2026-08-27 | QA-5/QA-8.99 proxy-settings application operation deduplication | ae74593 | Centralized per-profile proxy application and VS Code synchronization while preserving running-profile filtering, warning behavior, and log messages; focused ProxySettingsService suite passes 8/8 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,193 host and 69 webview tests, 418 architecture-checked production files, host c8 89.78%/82.79%/85.97%, Graphify 5,174 nodes/14,624 raw edges/12,465 normalized directed post-build edges, and Repowise 8.93 average / 6.78 hotspot health with 370 retained coverage files at 90.2%/82.2%; remaining low-cohesion and nested-complexity findings remain tracked |
| 2026-08-27 | QA-5/QA-8.100 proxy-settings responsibility separation | cc1f4fb | Separated profile-wide application, restoration, and backup inspection behind application services and a domain window-configuration port; narrowed ProxyManager and Accounts panel consumers to dedicated interfaces; focused proxy-settings/integration suites pass 15/15 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,193 host and 69 webview tests, 424 architecture-checked production files, host c8 89.78%/82.79%/85.97%, clean-archive Graphify 5,213 nodes/15,485 raw edges/13,491 normalized directed post-build edges, and Repowise 8.93 average / 6.82 hotspot health with 374 retained coverage files at 90.2%/82.2%; facade cohesion and adapter coverage remain tracked |
| 2026-08-27 | QA-8.101 proxy-window adapter coverage | 4ddfb2f | Added direct contracts for all synchronized and cleared VS Code proxy settings and the global configuration target; focused adapter suite passes 3/3 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,209 host and 69 webview tests, 424 architecture-checked production files, host c8 89.86%/82.82%/85.88%, clean-archive Graphify 5,216 nodes/15,495 raw edges/13,498 normalized directed post-build edges, and Repowise 8.94 average / 6.82 hotspot health with 374 retained coverage files at 90.2%/82.3%; both adapter files score 10.00/10 with no findings |
| 2026-08-27 | QA-5/QA-8.102 proxy-settings compatibility-facade removal | 3f1f58f | Removed the production `ProxySettingsService` facade, wired dedicated application/restoration/backup contracts directly from the composition root, and routed single-profile lifecycle application through `ProxySettingsApplicationService`; focused proxy-settings/integration/ProxyManager suites pass 17/17 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,210 host and 69 webview tests, 423 architecture-checked production files, host c8 89.85%/82.81%/86.04%, clean-archive Graphify 5,210 nodes/15,476 raw edges/13,480 normalized directed post-build edges, and Repowise 8.94 average / 6.82 hotspot health with 373 retained coverage files at 90.23%/82.23%; the removed facade no longer appears as a current production hotspot |
| 2026-08-27 | QA-5/QA-8.103 efficiency persistence read-adapter separation | b90554b | Split aggregate, event, and quota projections into separate injected persistence adapters while preserving the `EfficiencyDatabase` public facade and SQLite schema; focused persistence suites pass 7/7 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,210 host and 69 webview tests, 426 architecture-checked production files, host c8 90.13%/83.17%/86.36%, webview 93.96%/84.85%/92.55%, Graphify 5,242 nodes/14,808 raw edges/12,642 normalized directed post-build edges, and Repowise 8.94 average / 6.79 hotspot health with 376 retained coverage files at 90.48%/82.73%; separated readers score 9.52/9.85/10.00 and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.104 agent stream framing helper consolidation | 7bca506 | Centralized gzip/Connect candidate walking for direct and `HealthResponse` agent decoding, added a compressed-payload regression contract, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,210 host and 69 webview tests, 426 architecture-checked production files, host c8 90.14%/83.22%/86.36%, webview 93.96%/84.85%/92.55%, Graphify 5,243 nodes/14,812 raw edges/12,645 normalized directed post-build edges, and Repowise 8.94 average / 6.80 hotspot health with 376 retained coverage files at 90.49%/82.78%; decoder health is 7.26/10 with no new dependency-direction finding |
| 2026-08-27 | QA-5/QA-8.105 webview profile-action and proxy relationship boundaries | 238f5c2, cafd173 | Split profile actions into typed bridge/form hooks and centralized relationship merging across Bidi and agent-stream paths; complete Node 24.19.0/pnpm 10.34.0 audit with 1,210 host and 70 webview tests, 426 architecture-checked production files, host c8 90.16%/83.22%/86.32%, webview 94.19%/85.40%/92.73%, Graphify 5,257 nodes/14,861 raw edges/12,685 normalized directed post-build edges, and Repowise 8.94 average / 6.81 hotspot health with 379 retained coverage files at 90.54%/82.86%; extracted hook health 10.00/9.65/9.62 and safe-only dead-code empty |
| 2026-08-27 | QA-5/QA-8.106 extension window initialization boundary | 895da24 | Isolated current-window detection, existing proxy attachment, and accounts-panel auto-open decisions behind a focused composition workflow; added 3/3 lifecycle contracts; complete Node 24.19.0/pnpm 10.34.0 audit with 1,215 host and 70 webview tests, 426 architecture-checked production files, host c8 90.37%/83.20%/86.43%, webview 94.19%/85.40%/92.73%, Graphify 5,262 nodes/15,645 raw edges/13,635 normalized post-build edges, and Repowise 8.95 average / 6.81 hotspot health with 379 retained coverage files at 90.72%/82.94%; initialization health 9.53/10 and safe-only dead-code empty |
| 2026-08-27 | QA-5/QA-8.107 active conversation status presentation boundary | 8470687, a4a60c7, 9294c46 | Moved active-conversation status formatting, tooltip composition, warning decisions, and shared token/cost formatting out of the VS Code adapter; added pure presentation and formatter contracts, including empty, active, context, billed/live, sub-cent, and boundary cases; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,217 host and 70 webview tests, 428 architecture-checked production files, host c8 90.54%/83.56%/86.49%, webview 94.19%/85.40%/92.73%, Graphify 5,273 nodes/15,693 raw edges/13,686 normalized post-build edges, and Repowise 8.96 average / 6.83 hotspot health with 381 retained coverage files at 90.9%/83.2%; the presentation boundary scores 10.00/10 with no findings and safe-only dead-code analysis remains empty |
| 2026-08-27 | QA-5/QA-8.108 profile proxy edit application boundary | 8aeef42, 3559c81, 1e0d366 | Moved profile-edit proxy side effects out of the Accounts panel adapter into injected application services, made settings restoration explicit, removed the duplicate production restore path, decomposed lifecycle branches into modeled runtime actions, and added 11 direct/integration contracts; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,227 host and 70 webview tests, 430 architecture-checked production files, host c8 90.56%/83.58%/86.50%, webview 94.19%/85.40%/92.73%, Graphify 5,289 nodes/15,731 raw edges/13,718 normalized post-build edges, and Repowise 383 retained coverage files at 90.9%/83.2%; the use case scores 10.00/10, lifecycle service 9.85/10 with one low-severity DRY heuristic, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-8.109 EfficiencyAnalyzer facade coverage and unknown-error contract | b20eb8e | Added direct `EfficiencyAnalyzer` contracts for workflow delegation, queue deduplication, typed classifier failures, and arbitrary runtime rejection fallback; focused suite passes 4/4 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,233 host and 70 webview tests, 430 architecture-checked production files, host c8 90.60%/83.65%/86.68%, webview 94.19%/85.40%/92.73%, Graphify 5,296 nodes/15,751 raw edges/13,735 normalized post-build edges, Repowise retains 383 coverage files at 90.9%/83.2%, direct analyzer LCOV is 100.00% lines/branches, and safe-only dead-code remains empty |
| 2026-08-27 | QA-4/QA-8.110 QuotaClient fallback, merge, and HTTP error coverage | 929b9d8, 4bee4f2 | Added direct contracts for empty/unreadable IDE error bodies, IDE-to-web fallback, IDE preference when web is unavailable, successful source merging, and normalized dual-source failure; focused quota fetch suite passes 10/10 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,239 host and 70 webview tests, 430 architecture-checked production files, host c8 90.69%/83.79%/86.78%, webview 94.19%/85.40%/92.73%, Graphify 5,297 nodes/15,752 raw edges/13,736 normalized post-build edges, Repowise retains 383 coverage files at 90.9%/83.2%, QuotaClient LCOV is 100.00% lines/branches/functions, and safe-only dead-code remains empty |
| 2026-08-27 | QA-6/QA-7/QA-8.111 CertificateManager recovery-path contracts | 23d8f05 | Added direct contracts for rebuilding a missing public key from an existing certificate and regenerating material from an unparsable certificate; focused CertificateManager suite passes 8/8 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,241 host and 70 webview tests, 430 architecture-checked production files, host c8 90.73%/83.80%/86.78%, webview 94.19%/85.40%/92.73%, Graphify 5,297 nodes/15,752 raw edges/13,736 normalized directed edges, Repowise retains 383 coverage files at 90.9%/83.2%, CertificateManager direct coverage is 98.39% lines/95.83% branches, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.112 protobuf analysis CLI executable-entrypoint contracts | f03e423 | Added explicit `protobufjs` imports to both protobuf-analysis CLIs and smoke-tested their executable entrypoints against empty log directories; focused script suites pass 4/4 and 6/6, complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,241 host and 70 webview tests, 430 architecture-checked production files, host c8 90.73%/83.80%/86.78%, webview 94.19%/85.40%/92.73%, Graphify 5,299 nodes/15,760 raw edges/13,738 normalized directed edges, Repowise retains 383 coverage files at 90.9%/83.2%, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.113 webview diagnostic-bridge coverage | e936804 | Added direct contracts for typed diagnostic forwarding and early-boot logging when the inline VS Code API is unavailable; webview suite passes 71 tests across 18 files with 94.49%/85.46%/93.93% coverage, the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,241 host tests, 430 architecture-checked production files, host c8 90.73%/83.80%/86.78%, Graphify 5,299 nodes/15,761 raw edges/13,739 normalized directed edges, Repowise retains 383 files at 91.1%/83.3%, and the adapter reaches 100.00% direct line/branch/function coverage |
| 2026-08-27 | QA-1/QA-5/QA-8.114 Accounts panel dispatcher coverage | 3ef9ecd | Added a typed dispatcher contract for every specialized webview action and the unknown-message no-op; focused dispatcher suite passes 1/1 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,242 host and 71 webview tests, 430 architecture-checked production files, host c8 90.83%/84.08%/86.78%, webview 94.50%/85.47%/93.94%, Graphify 5,302 nodes/15,772 raw edges/13,747 normalized directed edges, Repowise retains 383 files at 91.1%/83.3%, the dispatcher reaches 100.00% direct line/branch coverage, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.115 Accounts panel provider lifecycle coverage | 97e230a | Added explicit provider contracts for panel disposal, no-panel reveal, runtime-ready refreshes, visible/hidden panel transitions, workspace-change refreshes, empty-workspace auto-opening, and missing-bundle diagnostics; focused provider suite passes 25/25 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host and 71 webview tests, 430 architecture-checked production files, host c8 90.92%/84.11%/87.02%, webview 94.50%/85.47%/93.94%, Graphify 5,306 nodes/15,776 raw edges/13,751 normalized directed edges, Repowise retains 383 files at 91.1%/83.3%, provider direct coverage reaches 95.66% lines/95.45% branches, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.116 ProxyManager construction and branch-contract hardening | e874784, d4dd93a | Replaced the long positional ProxyManager constructor with one typed options boundary, updated the composition root and all consumers, and added contracts for current-window proxy detection, tracking-service delegation, and unavailable proxy URLs; the updated ProxyManager facade suite passes 2/2 and the five-suite construction migration validation passed 10/10, the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host and 71 webview tests, 430 architecture-checked production files, host c8 90.94%/84.18%/87.08%, webview 94.49%/85.46%/93.93%, Graphify 5,307 nodes/15,002 raw edges/12,806 normalized directed edges, Repowise reports 6.41/10 for ProxyManager with 97.48% lines/100.00% branches over 357 coverable lines, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-4/QA-5/QA-8.117 session-token accounting and report-rendering boundaries | 3eb2f58, d5e87d8 | Extracted session state accounting/report calculations into `scripts/lib/session-token-summary.mjs` and pure text rendering into `scripts/lib/session-token-report.mjs`; mandatory session-token contract suite passes 3/3, including duplicate-event handling and the empty-capture CLI smoke path; complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host and 71 webview tests, 430 architecture-checked production files, host c8 90.94%/84.18%/87.08%, webview 94.49%/85.46%/93.93%, Graphify 5,318 nodes/15,047 raw edges/12,844 normalized directed edges, Repowise 8.95 average / 6.88 hotspot health with session summary/report modules at 9.15/8.78, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-4/QA-5/QA-8.118 session-capture analysis boundary | f05bbe1 | Extracted JSONL parsing, RPC/event classification, protobuf/body decoding, stream processing, and file-level orchestration into `scripts/lib/session-token-capture-analysis.mjs`; the focused session suite passes 5/5 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host and 71 webview tests, 430 architecture-checked production files, host c8 90.94%/84.18%/87.08%, webview 94.49%/85.46%/93.93%, Graphify 5,326 nodes/15,078 raw edges/12,872 normalized directed edges, Repowise 8.95 average / 6.91 hotspot health, capture analysis at 9.00/10 and the executable CLI at 9.65/10, and safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.119 webview message-state family boundaries | 729d69c | Split the webview application-message reducer into typed host, proxy, storage, and pricing family modules while preserving the reducer contract; the webview suite passes 74 tests across 18 files with 95.39% lines, 89.53% branches, and 94.15% functions, and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host tests, 430 architecture-checked production files, and host coverage at 90.94%/84.18%/87.08%; Graphify reports 5,344 nodes, 15,137 raw edges, and 12,921 normalized directed edges, Repowise reports 8.97 average / 6.91 hotspot health and 388 retained coverage files at 91.36%/84.06%, with the public reducer at 9.34/10 and each runtime family module at 9.65/10; safe-only dead-code remains empty |
| 2026-08-27 | QA-1/QA-5/QA-8.120 ProfileCard identity and actions boundaries | d178509 | Separated profile identity/metadata rendering from launch, menu, confirmation, and outside-click lifecycle behavior while keeping ProfileCard as a presentation compositor; the webview suite passes 77 tests across 18 files with 96.13% lines, 90.64% branches, and 94.38% functions, and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,250 host tests, 430 architecture-checked production files, and host coverage at 90.94%/84.18%/87.08%; Graphify reports 5,350 nodes, 15,164 raw edges, and 12,942 normalized directed edges, Repowise reports 8.98 average / 6.96 hotspot health and 390 retained coverage files at 91.44%/84.15%, with `ProfileCard.tsx` at 6.15/10, `ProfileCardActions.tsx` at 9.65/10, and `ProfileCardIdentity.tsx` at 10.00/10; safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-6/QA-7/QA-8.121 efficiency database lifecycle and recovery boundary | 7a58baf, 869c9fb | Moved schema initialization, migration recovery, database sizing, and destruction behind `EfficiencyDatabaseLifecycle`, added pre-probe WAL/SHM recovery snapshots so SQLite probing cannot erase recoverable sidecars, and added direct fresh/existing/corrupted/sidecar/size/destroy contracts; focused lifecycle suite passes 4/4 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 431 architecture-checked production files, host c8 91.00%/84.31%/87.15%, webview 96.13%/90.64%/94.38%, Graphify 5,363 nodes/15,204 raw edges/12,976 normalized directed edges, and Repowise 8.97 average / 6.92 hotspot health with 391 retained coverage files at 91.48%/84.19%; lifecycle health is 8.0/10 and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.122 proxy traffic usage coordination boundary | 792fd65 | Split profile resolution, local ingestion, shared-runtime notification, and diagnostic logging behind focused coordinator methods; replaced concrete tracking-service/global-logger dependencies with injected ingestion and logger capabilities; focused coordinator suite passes 6/6 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 431 architecture-checked production files, host c8 91.00%/84.31%/87.15%, webview 96.13%/90.64%/94.38%, Graphify 5,363 nodes/15,204 raw edges/12,976 normalized directed edges, and Repowise 8.97 average / 6.92 hotspot health; coordinator health is 7.8/10 with a test file and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.123 profile import feedback presentation policy | 25f2f1c | Extracted localized import counts, refresh eligibility, and success/error message selection into a pure presentation policy; direct policy suite passes 3/3 and the existing profile-handler suite passes 5/5, while the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 432 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,379 nodes/15,248 raw edges/13,012 normalized directed edges, and Repowise 8.97 average / 6.92 hotspot health; policy health is 9.8/10 and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.124 protobuf descriptor extraction responsibility boundaries | e76e4d6, eb250bf | Split protobuf message-factory, message-reference, enum, and service extraction into focused pure helpers, added malformed and unsupported-fragment contracts, and preserved package filtering and descriptor output compatibility; focused extraction suite passes 7/7 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 432 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,385 nodes/15,265 raw edges/13,028 normalized directed edges, and Repowise 8.97 average / 6.94 hotspot health with 392 retained coverage files at 91.5%/84.4%; parser health is 8.2/10 and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.125 protobuf JSONL routing and body-evaluation boundaries | 09b9ec1, 2967be4 | Separated pure protobuf-body evaluation, route/content classification, and explicit runtime contexts from verification-report mutation; focused verifier suite passes 8/8 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 432 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,387 nodes/15,271 raw edges/13,034 normalized directed edges, and Repowise 8.97 average / 6.97 hotspot health with 392 retained coverage files at 91.5%/84.4%; the verifier retains blocking filesystem and malformed-input signals by design, and safe-only dead-code remains empty |
| 2026-08-27 | QA-5/QA-8.126 agent text-extraction boundaries | f497f96 | Split the offline Agent analysis barrel into Connect framing/decoding, recursive text-bucket classification, and stream/prefetch metric modules; reused the canonical legacy token estimator, added 3/3 direct contracts, and passed the complete Node 24.19.0/pnpm 10.34.0 audit with 1,257 host and 77 webview tests, 432 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,410 nodes/15,341 raw edges/13,097 normalized directed edges, and Repowise 8.98 average / 6.97 hotspot health with 392 retained coverage files at 91.5%/84.4%; extracted-module health is 9.20/9.85/9.85 and the compatibility barrel is 10.00/10, with only intentional cross-tooling duplication/recovery-catch signals remaining and safe-only dead-code empty |
| 2026-08-27 | QA-5/QA-8.127 proxy traffic entry-analysis boundaries | 616376e | Separated JSON/protobuf/gzip entry decoding, nested insight aggregation, and JSONL file orchestration while preserving the public analysis API; the focused proxy traffic suite passes 4/4 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 432 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,425 nodes/15,377 raw edges/13,130 normalized directed edges, and Repowise 9.00 average / 6.92 hotspot health with 392 retained coverage files at 91.5%/84.4%; decoder, insight, and orchestration boundaries score 10.00/10, with only the intentional offline-CLI synchronous-I/O marker remaining and safe-only dead-code empty |
| 2026-08-27 | QA-5/QA-8.128 Accounts panel proxy action boundaries | 29b5baa | Split the Accounts panel proxy facade into dedicated runtime, certificate, and output adapters while preserving the compatibility API; added 10 direct behavior contracts covering start/stop decisions, missing profiles, failure handling, certificate guide/install/uninstall/save paths, and VS Code output commands; the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,257 host and 77 webview tests, 435 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,457 nodes/15,450 raw edges/13,197 normalized directed edges, and Repowise 9.00 average / 6.91 hotspot health with 395 retained coverage files at 91.9%/84.5%; the output adapter has no findings, runtime/certificate cohesion and cross-module duplication warnings are recorded as heuristic review signals, and safe-only dead-code remains empty |
| 2026-08-28 | QA-5/QA-8.129 proxy runtime composition coverage | abb66cc | Added a direct composition contract for the development-mode tracking, multi-profile database, extension-path, user-to-profile mapping, and injected-clock branches of `createProxyServerRuntime`, while retaining the production/no-tracking shutdown contract; the focused proxy-server suite passes 9/9 and the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,275 host and 77 webview tests, 435 architecture-checked production files, host c8 91.03%/84.40%/87.16%, webview 96.13%/90.64%/94.38%, Graphify 5,457 nodes/15,450 raw edges/13,197 normalized directed edges, and Repowise 9.00 average / 6.91 hotspot health with 395 retained coverage files at 91.9%/84.6%; safe-only dead-code remains empty and the remaining physical-recovery, native-matrix, supply-chain-attribution, and release-rehearsal gates remain open |
| 2026-08-28 | QA-5/QA-6/QA-7/QA-8.130 agent tracking repository-pool concurrency boundary | ed9eb9e | Made per-profile repository initialization single-flight, waited for pending initialization in profile/global close operations, cleared failed pending state for retry, and added 4/4 SQLite integration contracts for unknown profiles, concurrent sharing, close/reopen behavior, in-flight close, and initialization failure recovery; the official Node 24.19.0/pnpm 10.34.0 audit passes with 1,279 host and 77 webview tests, 435 architecture-checked production files, host c8 91.57%/84.66%/88.17%, webview 96.13%/90.64%/94.38%, Graphify 5,461 nodes/15,463 raw edges/13,205 normalized directed edges, and Repowise 9.00 average / 6.91 hotspot health with 395 retained coverage files at 92.0%/84.7%; the pool reaches 96.93% lines/93.33% branches/100.00% functions, safe-only dead-code remains empty, and physical-recovery, native-matrix, attribution, and release-rehearsal gates remain open |
| 2026-08-28 | QA-5/QA-8.131 efficiency and profile-launch presentation boundaries | 3c526c4, de7943a | Moved model-efficiency formatting, score-threshold/recommendation decisions, and notification requests into a VS Code-free application policy; moved profile launch quick-pick construction into a VS Code-free presentation policy; added 11/11 efficiency policy/adapter contracts, 10/10 profile policy/command contracts, and a related profile-command regression suite of 25/25; the complete Node 24.19.0/pnpm 10.34.0 audit passes with 1,300 host and 77 webview tests, 437 architecture-checked production files, host c8 92.09%/84.77%/88.50%, webview 96.13%/90.64%/94.38%, Graphify 5,498 nodes/15,550 raw edges/13,278 normalized directed edges, and Repowise 9.01 average / 6.91 hotspot health with 397 retained coverage files at 92.5%/84.7%; the pure efficiency policy, efficiency adapter, and profile launch policy reach 100.00% line/branch coverage, the launch adapter reaches 100.00% lines/81.25% branches/100.00% functions, safe-only dead-code remains empty, and the physical-recovery, native-matrix, attribution, and release-rehearsal gates remain open |
