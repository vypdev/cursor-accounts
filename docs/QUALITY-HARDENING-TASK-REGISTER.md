...
|---|---|---|---|---|
| QA-0 | Baseline and finding reclassification | COMPLETE | `pnpm run audit` passes at `8488af2`; current Graphify/Repowise/dependency evidence is captured on 2026-08-27 after the certificate lifecycle boundary slice | Keep this baseline immutable and update it after every cross-cutting change |
...
| Latest implementation commit | 8488af2 |
...
The latest `pnpm run audit` passed on 2026-08-27 at commit `8488af2` with:
...
`8488af2` using `--code-only`, no clustering, and a single worker. It scanned
854 code files and contains 6,197 nodes and 15,209 raw edges; the directed
post-build diagnostic graph contains 13,187 edges.
...
retains 13,321 valid candidate edges and collapses them to 13,187 directed
...
The latest Repowise index covers the code tree after `8488af2`; the host and
...
index reports 330 files, 87.16% lines, and 80.15% branches.
...
- Repowise `0.45.0` accepts 331 LCOV entries (298 exact and 33 resolved) and
  retains 330 files at 87.16% lines and 80.15% branches. Global health is
  ...
  boundaries; history-based churn/duplication markers on the compatibility
  file are retained as non-structural prioritization signals;
- Graphify `0.9.48` code-only extraction scans 854 code files and reports
  6,197 nodes and 15,209 raw edges. Its directed diagnostic reports 13,321
  valid candidate edges, 13,187 post-build edges, 1,888 dangling endpoint
