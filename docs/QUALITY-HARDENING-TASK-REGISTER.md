...
| Latest implementation commit | 01efc11 |
...
| Graphify | 0.9.48 |
| Repowise | 0.45.0 |
| Architecture gate | 417 production TypeScript files; tests excluded by contract |
| Documentation gate | 50 Markdown files |

### Reproducible audit

The latest complete `pnpm run audit` passed on 2026-08-27 at commit `01efc11`
...
- 1,193 host tests and coverage of 89.71% lines, 82.68% branches, and 85.96%
  functions;
- architecture rules and fixtures: passed for 417 production TypeScript files;
...
`01efc11` using `--code-only` and no clustering. It scanned 864 code files and
contains 5,172 nodes and 14,614 raw edges; the directed post-build diagnostic
graph contains 12,456 edges.
...
retains 12,607 valid candidate edges and collapses them to 12,456 directed
...
The latest Repowise index covers the code tree after `01efc11`; the host and
...
370 file entries (337 exact and 33 resolved mappings); the retained coverage
index reports 369 files, 90.14% lines, and 82.21% branches.
...
maintainability at 9.53/10 overall with a 9.10/10 hotspot score, and
